...
6. **Header duplication.** Sprint 0 added Save/Restore/Download to the hub scratch action bar. The chat header still has its own snapshot controls and download button for sandbox mode. These overlap. **Recommendation: consolidate to hub-only** in Sprint 3f. The header is already crowded; the hub is the right surface for workspace-level actions.
