...
Status annotations were first reviewed against PR #72 (`cb4814f`) and then updated after follow-up dedupe refactors on `main`.

---

## 1. API Key / Config Hooks (5 files, highest duplication)

**PR #72 status:** ✅ Complete
...
Follow-up refactor extracted shared primitives in `hooks/useExpandable.ts` and `components/cards/expandable.tsx` (`ExpandChevron`, `ExpandableCardPanel`) and migrated expandable card implementations to use them.
...
Historical audit files: `FileCard`, `SandboxCard`, `DiffPreviewCard`, `PRCard`, `EditorCard`, `WorkflowLogsCard`, `CIStatusCard`, `FileSearchCard`.
Current shared-primitive adoption covers all cards that still use this simple pattern.
...
Follow-up refactor extracted shared status helpers in `components/cards/ci-status-helpers.tsx` and migrated `CIStatusCard`, `WorkflowRunsCard`, and `WorkflowLogsCard` to the shared tone/icon/color mapping.

Previously, `CIStatusCard`, `WorkflowRunsCard`, and `WorkflowLogsCard` each defined their own `checkIcon(status, conclusion)` and `headerBg(status)` / `headerColor(status)` mappers with the same success/failure/pending/cancelled logic.

---

## Summary

| Pattern | Instances | Spread across | PR #72 status |
|---------|-----------|---------------|---------------|
| API key hook skeleton | 5 | `hooks/use*Config.ts`, `hooks/useMoonshotKey.ts` | ✅ Complete |
| Provider model get/set | 3 | `lib/providers.ts` | ✅ Complete |
| `asRecord()` | 6 | `lib/*.ts` | ✅ Complete |
| Fenced-JSON detection loop | 7 | `lib/*-tools.ts`, `lib/coder-agent.ts`, `lib/tool-dispatch.ts` | ✅ Complete |
| Streaming timeout promise | 3 | `lib/auditor-agent.ts`, `lib/coder-agent.ts` | ✅ Complete |
| `timeAgo()` | 7 | `components/cards/*.tsx`, `sections/`, `components/chat/` | ✅ Complete |
| `isNetworkFetchError()` | 2 | `hooks/useGitHub*.ts` | ✅ Complete |
| `validateToken()` | 2 | `hooks/useGitHub*.ts` | ✅ Complete |
| Card shell class string | 21 | `components/cards/*.tsx` | ✅ Complete |
| Expand/collapse UI | 8 | `components/cards/*.tsx` | ✅ Complete |
| CI status icon/color mappers | 3 | `components/cards/CI*.tsx`, `Workflow*.tsx` | ✅ Complete |
