...
Date: 2026-05-07 (Phase 1 substrate + first-tool dispatch shipped 2026-05-11 → 2026-05-12; chat-layer wiring, tool-op fan-out, and UI half landed 2026-05-12; 1.f hardening, Phase 3 slices 1–4, Phase 2 storage decision (2.a), and Phase 2.b–2.f landed 2026-05-13; hash/token-id allowlist hardening landed 2026-05-13; Phase 2.j daemon-shell convergence onto the workspace shell landed 2026-05-28)
Status: **Phases 1, 2, and 3 shipped** — Phase 1.a–1.f (#507–#511, 3c.2a/b, 3c.3, auto-reconnect 2026-05-13) and Phase 3 slices 1–4 (#518–#521) landed. Phase 2 storage shape locked 2026-05-13: DO in-memory ring buffer, no durable storage (see Q#2 + Phase 2 sub-phase table). Phase 2.b (#524), 2.c (#525), 2.d.1 (#526), 2.d.2 (#528), 2.e (#529), and 2.f (#530) all landed 2026-05-13. **The relay path is end-to-end testable from a phone surface as of #530 — the milestone the track was built around.** Hash/token-id allowlist hardening landed 2026-05-13 (2.g, #531). Long-lived WS reuse for chat dispatch landed 2026-05-13 (2.h, #532): chat-layer `sandbox_*` tool calls now route through the hook-owned WebSocket via a `LiveDaemonBinding` instead of opening a transient WS per call. DaemonChatBody extraction + DaemonModelPicker rename landed 2026-05-13 (2.i below): the two near-clone chat screens collapsed to thin wrappers around a shared `DaemonChatBody` and the picker dropped its misleading `LocalPc` prefix. Phase 2.j daemon-shell convergence landed 2026-05-28 (#668–#673): remote sessions now mount the standard workspace shell, the standalone `DaemonModelPicker` dissolved in favor of Settings access wired directly into the daemon hub, per-daemon appearance + CLI-session affordances moved into shared hooks (`useDaemonAppearance`, `useDaemonSettingsBundles`, `useDaemonCliSessions`), the daemon compose row adopted the full `ChatInput`, and `list_sessions` now persists session mode for downstream consumers. Remaining: Phase 4 (desktop wrapper polish).
Owner: Push
Related: `docs/cli/design/Push Runtime Protocol.md`, `docs/archive/decisions/Web and CLI Runtime Contract.md`, `docs/archive/decisions/push-runtime-v2.md`, `docs/archive/decisions/Diff and Annotation Envelope.md`

## Shipping status

The shipped scope diverged from the original Phase 1 enumeration in one way: the connector substrate is more complete than spec'd (full WS adapter, paired-device UI, mode chip, IndexedDB storage, dispatch seam). Otherwise the track now matches the original Phase 1 vision end-to-end: a paired Local PC session has a real chat, the chat round loop routes sandbox tool calls through the daemon (`sandbox_exec` / `sandbox_read_file` / `sandbox_write_file` / `sandbox_list_dir` / `sandbox_diff`), the Stop button halts the turn AND interrupts the in-flight daemon child via `cancel_run`, and a dropped WS auto-reconnects with exponential backoff up to 5 attempts before surfacing a manual Retry banner. Phase 3 (permission + audit model + `submit_approval` UI) shipped via #518–#521. Phase 2 storage decision locked 2026-05-13 (Q#2; see Phase 2 sub-phase table); Phase 2.b–2.f code track shipped 2026-05-13 (#524–#530); Phase 2.g hash/token-id allowlist hardening (#531) closed the daemon-restart limitation called out in 2.e/2.f; Phase 2.h long-lived WS reuse for chat dispatch (#532) closed the per-call transient-WS overhead deferred from 2.f; Phase 2.i DaemonChatBody extraction (2026-05-13) closed the near-clone screen duplication and renamed `LocalPcModelPicker` → `DaemonModelPicker`. Phase 2.j daemon-shell convergence (#668–#673, 2026-05-28) walked past 2.i: the standalone `DaemonModelPicker` was deleted in favor of routing Settings into the daemon hub directly, remote sessions now mount the standard workspace shell, and the duplicated chat-shell affordances (appearance toggle, Palette button, CLI-sessions drawer row, full ChatInput) moved behind three shared hooks. Phase 4 desktop wrapper still open.
...
| 2 — Worker-mediated relay | Outbound-from-PC WebSocket to a Worker/DO that pairs phone client to daemon. Storage shape decided 2026-05-13 (DO in-memory ring buffer, Q#2 below); sub-phase breakdown drafted in the Phase 2 sub-phase table below. | Shipped 2026-05-13 (2.a–2.f) — end-to-end testable from a phone surface | #524 · #525 · #526 · #528 · #529 · #530 |
...
| 2.d.2 — Ring buffer + replay | Per-DO `Map<seq, envelope>` capped by both count (default `N=256`) and age (default `Y=60s`); first-to-fire evicts. After auth the phone client sends an initial `relay_attach` envelope carrying its last-seen `seq` (schema landed in 2.d.1). Relay replays envelopes after that `seq` if still buffered; a gap larger than the buffer emits `relay_replay_unavailable` so the client falls back to `attach_session` for current state rather than silently missing envelopes. Defaults overridable by env (`PUSH_RELAY_BUFFER_COUNT`, `PUSH_RELAY_BUFFER_AGE_MS`). | Shipped 2026-05-13 | #528 |
| 2.e — pushd outbound dial + reconnect | `cli/pushd-relay-client.ts` opens an outbound WS to the Worker; reuses the Phase 1.f reconnect ladder `[1s, 2s, 4s, 8s, 16s, 30s]` capped at 6 attempts (matches `useLocalDaemon.ts` so the 30s tier is reachable — briefing's "cap 5" was an outdated literal). `push daemon relay enable\|disable\|status` CLI surface, live admin RPCs (`relay_enable` / `relay_disable` / `relay_status`) so config changes take effect without a daemon restart. Audit events for `relay.connect` / `relay.disconnect` via the Phase 3 slice 3 audit log. **Origin-gate carve-out is bearer-prefix-scoped**, not auth-presence-scoped: `pushd_relay_*` skips `validateOrigin` (pushd is Node, not a CSRF target); `pushd_da_*` (phone) still enforces origin. pushd emits `relay_phone_allow` / `relay_phone_revoke` envelopes from `mint_device_attach_token` / `revoke_device_attach_token` / cascade, and re-emits the full allowlist on every `relay.connect` so a DO restart mid-session recovers. New `~/.push/run/pushd.relay.json` (chmod 0600) holds the deployment-scoped relay token. **Daemon-restart limitation closed by 2.g** — the in-process registry is now reseeded from the persisted attach-token hash store at startup, so phones with still-valid attach tokens survive a daemon restart without re-pairing. | Shipped 2026-05-13 | #529 |
| 2.f — Phone client mode + UI | Web client gains a relay adapter (`app/src/lib/relay-daemon-binding.ts`) sibling to the localhost WS adapter from Phase 1, plus a `useRelayDaemon` hook sibling to `useLocalDaemon` (per the AGENTS.md "background tasks" guardrail — feature hooks ship as siblings, not appended to `useChat.ts`). Same `[1s,2s,4s,8s,16s,30s]` cap-6 reconnect ladder as `useLocalDaemon`. `relay_attach { lastSeq }` resumes replay after disconnects; `relay_replay_unavailable` triggers a transient amber flash on the mode chip (no banner). Hub adds a "Remote · Experimental" tile separate from "Local PC", gated on `VITE_RELAY_MODE`. New `push daemon pair --remote` CLI mints a one-shot bundled paste string (`push-remote.<base64url-json>` carrying deploymentUrl + sessionId + attach token); operator copies the line, pastes on the phone, the web pair panel decodes + verifies + persists. **Chat-tool dispatch is now transport-polymorphic**: `local-daemon-sandbox-client.ts` exports a `DaemonBinding = LocalPcBinding \| RelayBinding` union and `createTransientAdapter` picks the WS constructor by shape (`'deploymentUrl' in binding` → relay), so every `sandbox_*` daemon helper works through either transport. IndexedDB schema bumped to v6 with a new `paired_remotes` store. **Deferred follow-ups (called out in PR header):** long-lived WS reuse for chat dispatch (today a transient WS opens per call); extracting a generic `DaemonChatBody` from the two near-clone chat screens; renaming `LocalPcModelPicker` → `DaemonModelPicker`. Daemon-restart limitation from 2.e is closed by 2.g (below). | Shipped 2026-05-13 | #530 |
| 2.g — Hash/token-id allowlist hardening | `relay_phone_allow` / `relay_phone_revoke` envelopes carry `tokenHashes` (`sha256(bearer)` base64url-encoded) instead of bearer plaintext. The DO's `allowedPhoneTokenHashes` is a `Set<hash>`; phone bearers are hashed once at WS upgrade via WebCrypto and stored as `meta.bearerHash` so every forward gate is an O(1) set lookup. pushd's in-process allowlist is reseeded at startup from `listDeviceAttachTokens()` (lazy-expired-filtered) before the relay client dials, so the first `relay_phone_allow` re-emit on connect carries every still-valid attach token — closing the daemon-restart limitation called out in 2.e and 2.f. Hard cutover on the envelope shape: old pushd talking to a new Worker fails the validator (frame dropped), which is the loud failure mode we want for a deploy mismatch. New `seedAllowlistFromAttachTokens(registry, listFn)` factored out of pushd.ts so the boot wire-up has a unit test; cross-impl hash compatibility (node `createHash` ↔ Worker WebCrypto) pinned in `app/src/worker/relay-do.test.ts`. Closes Q#9. | Shipped 2026-05-13 | #531 |
| 2.h — Long-lived WS reuse for chat dispatch | Chat-layer `sandbox_*` tool calls now route through the hook-owned long-lived WebSocket instead of opening a transient WS per call (the deferred follow-up called out in #530). New `LiveDaemonBinding` shape (`{ params, request }`) bundles connection params with a `request` fn bound to the long-lived WS that `useLocalDaemon` / `useRelayDaemon` already own. The hooks expose a memoized `liveBinding` field (null until the WS reaches `open`); `LocalPcChatScreen` / `RelayChatScreen` pass it to `setLocalDaemonBinding`, which now accepts `DaemonBinding \| LiveDaemonBinding`. `runWithBinding(binding, fn, opts)` is the single entry point for the per-tool helpers — when the binding has a `request` fn it routes through it directly; otherwise it falls back to `withTransientBinding` (preserved for pairing-probe paths like `identifyLocalDaemon` and the attach-token mint flow). Abort handling on the live path fires `cancel_run` on the same long-lived WS and rejects with `AbortError`, mirroring the post-open semantics of the transient path; the hook owns the WS lifecycle, so this path never closes the connection. AGENTS.md "one source of truth" guardrail respected — `ToolDispatchBinding` is the single shape that flows from `useChat.localDaemonBindingRef` → `chat-tool-execution` → `executeAnyToolCall` → per-tool helpers. | Shipped 2026-05-13 | #532 |
| 2.i — DaemonChatBody extraction + DaemonModelPicker rename | `LocalPcChatScreen` and `RelayChatScreen` collapse to ~60 lines each, mounting the right daemon hook (`useLocalDaemon` / `useRelayDaemon`) and delegating the shared shell to a new `DaemonChatBody` (`app/src/components/daemon/DaemonChatBody.tsx`). The body owns `useChat(null)`, conversation init, workspace context, compose state, ReconnectBanner, ApprovalPrompt placement, ChatContainer wiring, and the layout — everything that used to be duplicated 95% verbatim. Approval state extracted into a shared `useApprovalQueue` hook (`app/src/hooks/useApprovalQueue.ts`) with `classifyApprovalEvent` as a pure-helper testable surface. `components/local-pc/` → `components/daemon/`; `LocalPcModelPicker` renamed to `DaemonModelPicker` now that both transports use it (the prefix was misleading since 2.f). What stays per-transport, by design: the daemon hook mount (Rules-of-Hooks and divergent return shapes — relay's `replayUnavailableAt` doesn't exist on the local-PC hook), the mode chip (`LocalPcModeChip` vs `RelayModeChip` passed as a `ReactNode`), unpair storage (`clearPairedDevice` vs `clearPairedRemote`), and the workspace context build. Decision-doc note for future polish: the existing `LocalPc-` prefixed util `buildLocalPcWorkspaceContext` still serves both transports through its `transport` arg; renaming it is a separate, low-value churn we're not doing here. | Shipped 2026-05-13 | #533 |
| 2.j — Daemon-shell convergence onto the workspace shell | Six-PR cluster that walked past 2.i: (a) **#668** unifies remote sessions onto the standard workspace shell — `DaemonHubSheet` deleted, the daemon path now reuses the same chrome the cloud-sandbox shell uses, with daemon-specific affordances composed in via hooks rather than a parallel sheet. (b) **#669** extracts per-daemon appearance + Palette-button state into `useDaemonAppearance`. (c) **#670** plumbs Settings access into the daemon hub directly, which made the standalone `DaemonModelPicker` redundant — both `DaemonModelPicker.tsx` and its test file were deleted, with `useDaemonSettingsBundles` (~460 lines) owning provider/model + system-prompt bundle wiring instead. (d) **#671** swaps the minimal daemon compose row for the full `ChatInput` so the daemon shell gets the same attachment + slash-command + tab-complete affordances as the cloud surface. (e) **#672** persists session `mode` on the daemon's `list_sessions` envelope so downstream consumers (drawer, hub) can route by transport. (f) **#673** surfaces the daemon's CLI sessions in the mobile drawer via a new `useDaemonCliSessions` hook + `drawer-cli-row` component. Net: ~−530 lines of daemon-specific UI, three new shared hooks, and the daemon path is now a thin composition over the workspace shell rather than a parallel screen. | Shipped 2026-05-28 | #668, #669, #670, #671, #672, #673 |
...
8. Diff/review payload shape across the relay: when a phone surface attaches to a remote `pushd` daemon and Reviewer/Auditor/Coder emit diff + annotations, what envelope rides the relay? **Tracked separately** in `docs/archive/decisions/Diff and Annotation Envelope.md` (Draft, 2026-05-12). The decision there is to hand-roll the envelope in `lib/` and treat `@pierre/diffs` and `modem-dev/hunk` as design references only; this keeps remote sessions on `push.runtime.v1` without inventing a parallel vocabulary for review payloads.
9. ~~Allowlist persistence: 2.e shipped with the relay phone allowlist in process memory only, so a daemon restart dropped every entry and forced phones with still-valid attach tokens to re-pair before forwarding could resume. Hash-based or tokenId-based persistence to fix this?~~ **Answered (2.g, 2026-05-13)**: hashed-tokenId allowlisting. The wire and DO switched from bearer plaintext to `sha256(bearer)` base64url-encoded (`relay_phone_allow.tokenHashes` / `relay_phone_revoke.tokenHashes`). The DO computes the hash once at WS upgrade via WebCrypto and gates forwarding/replay on `meta.bearerHash`. pushd reseeds its in-process registry from `listDeviceAttachTokens()` at startup (filtered for expiry), so the first `relay_phone_allow` re-emit on relay connect carries every still-valid attach token. The persisted attach-token store has always held only hashes — this slice is what made the wire match. Hard cutover on the envelope shape; a stale pushd talking to a new Worker fails the validator (loud failure, not silent allowlist miss). Cross-impl hash compatibility (node `createHash('sha256').digest('base64url')` ↔ Worker WebCrypto + base64url-replaced btoa) is pinned in the DO test suite.
