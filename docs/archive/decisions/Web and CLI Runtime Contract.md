...
This is fully converged as of 2026-05-28. The kernel `createToolDispatcher` in `lib/tool-dispatch.ts` owns fenced + bare extraction and malformed-call reporting on both surfaces. Web's `detectAllToolCalls` (`app/src/lib/tool-dispatch.ts`) and CLI's `detectAllToolCalls` (via `cli/tools.ts:PASS_THROUGH_CLI_SOURCE`) both route through the same kernel for those phases. Per-turn phase grouping (reads → mutations batch → trailing side-effect) lives in shared `lib/tool-call-grouping.ts:groupCallsByPhase`. Both surfaces emit the same structured error codes — `FILE_MUTATION_BATCH_OVERFLOW` for batch overflow, `MULTI_MUTATION_NOT_ALLOWED` for ordering violations. Recovery shapes (namespaced `functions.<name>:<id>` + XML `<tool_call>`) are handled by `recoverNamespacedToolCalls` / `recoverXmlToolCalls` in `lib/`: CLI lets the kernel run those internally via the default `enableInternalRecovery: true`; web sets `enableInternalRecovery: false` and runs the recovery functions itself in a Phase 2 wrapped by a `!hasExplicitWrappers` gate, so heuristic recovery never competes with a canonical wrapper. See [Tool-Call Parser Convergence Gap](Tool-Call%20Parser%20Convergence%20Gap.md) for the six-PR resolution narrative (#677, #679, #680, #681, #683, #684).
...
`lib/protocol-schema.ts` is the canonical runtime validator for the daemon's wire envelopes and the delegation event payloads (`subagent.*`, `task_graph.*`, and relay-control envelopes). It began in `cli/protocol-schema.ts` on 2026-04-14 and moved to `lib/` during the Remote Sessions work because the web Local PC / Remote adapters consume the same wire format directly. It enforces the `SessionEvent` contract defined in `cli/session-store.ts` — including the explicit "`runId` must be omitted, never `null`" rule — and is wired into `broadcastEvent` under a `PUSH_PROTOCOL_STRICT=1` env var so the daemon-integration test harness fails on drift.

The shapes themselves come from the shared `RunEventInput` union in `lib/runtime-contract.ts` plus relay envelope definitions in the validator. Drift-guard tests re-extract those literals from the contract source at test time so anyone adding a new delegation variant breaks both sides' tests in lockstep.
