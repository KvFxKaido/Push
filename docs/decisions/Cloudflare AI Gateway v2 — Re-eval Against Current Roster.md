...
- **Endpoint choice:** the gateway-host OpenAI-compat surface (`gateway.ai.cloudflare.com/v1/{account}/{slug}/compat/chat/completions`), NOT the `api.cloudflare.com/…/ai/v1` REST host this section originally named. Rationale: it reuses the exact `CF_AI_GATEWAY_*` env triplet already set in prod (zero new ops surface; rollback = the provider 401s dormant, same as Path 1), it rides `buildAiGatewayUrl` unchanged (`compat` is a first-party path segment), and it applies the named gateway's config — stored BYOK keys, unified billing, logs — which is the point. The REST host remains an alternative if `/compat` proves feature-poor.
...
- **Auth:** `CF_AI_GATEWAY_TOKEN` (Worker secret, Settings-saved token, or user-stored key) rides both `Authorization: Bearer` (the compat endpoint's OpenAI-SDK auth) and `cf-aig-authorization` (authenticated-gateway header). Upstream provider credentials resolve inside the gateway: per-request key → stored BYOK → Unified Billing.
- **Pending live verification** (code-complete, not yet run against a real gateway): the `workers-ai/@cf/...` id convention on /compat; whether Authorization alone suffices on an authenticated gateway; native tool-call + streaming fidelity per upstream (the "beige paste" checks: exact streaming semantics, strict tool schemas, reasoning controls, error shapes); the unified-billing per-token delta (also still open from Path 1). Note Unified Billing's 5% credit-purchase fee, and that Replicate remains BYO-token via its native API — the `env.AI.run("replicate/...")` milestone has not landed.
- **Not in this spike:** the two-tier unlock below (the spike is single-key: the CF token IS the provider's key); a live unified-catalog models proxy (curated seed + free-text only); CLI config surface for account/slug.
