...
Date: 2026-07-09
Status: **Current** — Path 1 shipped, **verified live, and active in production** 2026-07-09 (openai/anthropic/google → `push-gate`, 200s in CF logs; prod `CF_AI_GATEWAY_*` secrets confirmed set; see Open questions); Path 1.5 spiked — `ollama` + 4 custom providers verified live 2026-07-09 (`kilocode` dropped); Bucket B — `deepseek` wired (first-party `/anthropic` verified), `vertex` deferred then removed with Bucket D (#1378); **Path 2 spiked 2026-08-07** as the `cloudflare-gateway` provider (see "Path 2 spike" below) — code-complete against the gateway `/compat` endpoint, pending live verification; the two-tier unlock spec'd below remains design.
...
### Path 2 spike — `cloudflare-gateway` provider (2026-08-07)

Cloudflare's 2026-08-07 unification announcement (Workers AI + AI Gateway share one model-access path; 70+ third-party models on the unified catalog; prepaid Gateway credits can now pay for Workers AI frontier models — Kimi K2.6 / K2.7 Code / GLM-5.2 at 50 req/min vs 20 on Workers Paid) was the trigger this section was waiting for. Spiked as a fifteenth provider, **additive** — no existing adapter changed:

- **Endpoint choice:** the gateway-host OpenAI-compat surface (`gateway.ai.cloudflare.com/v1/{account}/{slug}/compat/chat/completions`), NOT the `api.cloudflare.com/…/ai/v1` REST host this section originally named. Rationale: it reuses the existing account/slug route and `buildAiGatewayUrl` unchanged (`compat` is a first-party path segment), while applying the named gateway's config — stored BYOK keys, unified billing, logs — which is the point. The route stays dormant until account + slug resolve; `/compat` adds one dedicated credential secret rather than overloading the existing gateway-auth token. The REST host remains an alternative if `/compat` proves feature-poor.
- **Wire:** plain `openai-compat` — the compat endpoint speaks Chat Completions SSE for every upstream, so the provider is a ~10-line `createOpenAIChatStream` leaf + one Worker proxy handler. Model ids are `{gateway-provider}/{model}` (`openai/gpt-5-mini`, `anthropic/claude-sonnet-4-5`, `workers-ai/@cf/zai-org/glm-5.2`); `lib/capability-profile.ts` splits the routing prefix off before model-family heuristics so tool-calling/structured-output gates key on the leaf.
- **Policy:** `initialFallbackEligible: false`, `failoverEligible: false` — routing a round here changes who gets billed, so the provider is only ever an explicit pick. Web-only for now (no `cli` block): the compat URL embeds account + slug, which the CLI has no config surface for yet.
- **Auth:** the two header roles resolve independently. `CF_AI_GATEWAY_COMPAT_TOKEN` (or a Settings/user-stored compat token) supplies `Authorization: Bearer` for the OpenAI-SDK surface. Existing `CF_AI_GATEWAY_TOKEN` supplies only `cf-aig-authorization` when the named gateway requires authentication. Neither token shadows or substitutes for the other. Upstream provider credentials resolve inside the gateway: per-request key → stored BYOK → Unified Billing.
- **Pending live verification** (code-complete, not yet run against a real gateway): the `workers-ai/@cf/...` id convention on /compat; the separate `Authorization` + `cf-aig-authorization` pairing on an authenticated gateway; native tool-call + streaming fidelity per upstream (the "beige paste" checks: exact streaming semantics, strict tool schemas, reasoning controls, error shapes); the unified-billing per-token delta (also still open from Path 1). Note Unified Billing's 5% credit-purchase fee, and that Replicate remains BYO-token via its native API — the `env.AI.run("replicate/...")` milestone has not landed.
- **Not in this spike:** the two-tier unlock below; a live unified-catalog models proxy (curated seed + free-text only); CLI config surface for account/slug.
...
- **Response-caching posture — decided 2026-07-22 (#1554 → #1573).** `push-gate` has had response caching enabled since creation (`cache_ttl: 300`, config `modified_at 2026-07-09`): an identical request body replays the stored response from the edge (`cf-aig-cache-status: HIT`, ~37ms, $0, **no token usage recorded** — so hits also punch a hole in cost/token telemetry). Real chat bodies grow monotonically and almost never repeat; the cache serves mostly the traffic it harms — repeated-prompt evals and BYOK probes, where a byte-identical replay fakes determinism (measured pre-fix: 1 HIT / 213 cacheable req over 24h, the hit replaying a streaming `functionCall`). Posture: **caching stays enabled in the gateway config; every Push runtime path bypasses it per-request** with `cf-aig-skip-cache: true` — the four Worker seams in #1573 (both proxy factories, `resolveAiGatewayFetchTarget`, `env.AI.run`), the four CLI transports via `lib/ai-gateway.ts` host detection. Eval/probe paths are additionally cache-proof by *assertion*, not just construction: the eval harness preflight (`scripts/eval/run-evals.ts`) sends two identical tiny calls through the transport's own header construction — spaced ~5s apart, because the edge cache write is async and a back-to-back repeat reads MISS even when caching is live (verified 2026-07-23) — and aborts the suite on a `HIT`. On a detected gateway route the bar is strict (non-2xx probe, missing `cf-aig-cache-status`, or a thrown probe all abort — unverified is failure where verification was possible); elsewhere only a verified `HIT` aborts, so a custom-domain gateway is caught while it forwards the standard cache-status header but a front that strips it is out of reach — run evals against the canonical `gateway.ai.cloudflare.com` host for the strict tier. Manual/keyless BYOK probes should send `cf-aig-skip-cache: true` explicitly (see the probe guidance in the memory/runbooks).
