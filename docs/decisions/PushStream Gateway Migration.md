...
Status: in_progress — provider side validated end-to-end, every agent role on PushStream; only Phase 9 (retire the bridges) remains
...
**Phase 9 — Retire `createProviderStreamAdapter` and `providerStreamFnToPushStream`.** Both bridges exist exclusively to handle the staged migration. With every agent role now on PushStream, the reverse bridge's only remaining users are the app-side wrappers (`auditor-agent.ts`, `reviewer-agent.ts`, `explorer-agent.ts`, `deep-reviewer-agent.ts`, `coder-agent.ts`, `planner-agent.ts`) and `cli/pushd.ts` — all wrapping a `ProviderStreamFn` returned by `getProviderStreamFn` / `createDaemonProviderStream`. The forward `createProviderStreamAdapter` is still used by every adapter-routed provider's `streamXChat` export. Phase 9 unwires both together: route `streamXChat` exports through native PushStreams (deletes the forward adapter), and refactor `getProviderStreamFn` to return a `PushStream` directly (deletes the reverse bridge and every per-shim `WeakMap` cache).

## Recommendation for next

**Phase 9 — retire `createProviderStreamAdapter` and `providerStreamFnToPushStream`.** With every agent role now consuming PushStream events, both bridges are only carrying the staged-migration boundary they were built for. The cleanup is two coordinated changes:

- Replace each provider-side `createProviderStreamAdapter(normalizeReasoning(<provider>Stream), ...)` wrapping with a direct call-site whose `streamXChat` export is no longer a `ProviderStreamFn` — orchestrator wiring (`getProviderStreamFn`) becomes the gateway-to-consumer seam instead of the adapter.
- Refactor `getProviderStreamFn` (web) and `createDaemonProviderStream` (CLI) to return a `PushStream` directly. Once that lands, the per-shim `WeakMap<ProviderStreamFn, PushStream>` caches in `auditor-agent.ts` / `reviewer-agent.ts` / `explorer-agent.ts` / `deep-reviewer-agent.ts` / `coder-agent.ts` / `planner-agent.ts` go away with the bridge, and `runAuditor` / `runReviewer` / etc. are called with the PushStream directly.

Sub-steps, in order:

- Convert `streamXChat` exports off the legacy callback shape (touch six providers + Cloudflare, but Cloudflare already iterates directly).
- Convert `getProviderStreamFn` and `createDaemonProviderStream` to return PushStreams.
- Delete every `bridgeStreamFn` / `resolve...PushStream` helper and its WeakMap cache.
- Delete `createProviderStreamAdapter` + `providerStreamFnToPushStream` from `lib/provider-contract.ts`.
- Drop the dead `PROVIDER_STREAM_CONFIGS` entries (can fold into the same PR).
