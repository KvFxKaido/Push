...
**Phase 6 (continued) — Migrate the remaining agent roles off the 12-arg callback.** Auditor is on PushStream (PR #389). Coder, Reviewer, Planner, Explorer, DeepReviewer still consume `ProviderStreamFn`. Each migration follows the Auditor template: replace the `streamFn?: ProviderStreamFn` option with `stream?: PushStream<LlmMessage>`, iterate via `iteratePushStreamText`, and cache bridged streams by `streamFn` identity at the app-side wrapper.
...
**Phase 9 — Retire `createProviderStreamAdapter` and `providerStreamFnToPushStream`.** Gated on every agent role being migrated to PushStream consumption. The adapter and its reverse bridge exist exclusively to handle the staged migration; once nothing calls them, delete both together.

## Recommendation for next

**Migrate the Coder agent off `ProviderStreamFn` (Phase 6 continued).** The full provider catalog now sits on PushStream + the shared `openAISSEPump` (six providers via the adapter, one direct), so the next bottleneck for retiring `createProviderStreamAdapter` is consumer-side migration, not provider-side. Coder is the highest-traffic agent role and the closest analog to Auditor; the migration follows the same template (`stream?: PushStream<LlmMessage>` option, `iteratePushStreamText` iteration, app-side `WeakMap` cache keyed by underlying `streamFn` to preserve any coalescing). After Coder lands, Reviewer / Planner / Explorer / DeepReviewer can sweep behind it.

Remaining sub-steps, in order:

- Migrate Coder. Capture any `iteratePushStreamText` shape gaps that the new role surfaces (Auditor only iterates `text_delta`; Coder may need `tool_call_delta` and `reasoning_delta` exposed too).
- Sweep the remaining agent roles (Reviewer / Planner / Explorer / DeepReviewer) onto the same pattern.
- Clean up the dead `PROVIDER_STREAM_CONFIGS` entries (small follow-up PR).
- Once every agent role consumes PushStream, retire `createProviderStreamAdapter` + `providerStreamFnToPushStream` (Phase 9).
