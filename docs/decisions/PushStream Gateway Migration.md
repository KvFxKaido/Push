...
**Phase 7 — Extract a shared SSE pump.** Three callers (`openrouterStream`, `zenStream`, `kilocodeStream`) now duplicate the buffer + line-split + OpenAI-shape delta parsing + native tool-call accumulation almost line-for-line. The duplication is concrete enough to justify extraction. The remaining Phase 8 ports (OpenAdapter, Nvidia, Blackbox) should happen _after_ the pump extraction so they benefit from it rather than compounding the duplication further.

**Phase 8 — Port the rest of the OpenAI-compatible catalog.** OpenAdapter, Nvidia, Blackbox remain on `streamProviderChat('<provider>', ...)`. Same mechanical shape as the Zen / Kilocode ports. Deferred until Phase 7 lands the shared pump.
