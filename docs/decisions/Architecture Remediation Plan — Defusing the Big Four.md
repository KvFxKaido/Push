...
4. ~~**Extract sandbox-tools verification family.**~~ **Landed 2026-04-15 in commit `14a63c4`** ("refactor(tools): extract sandbox verification family (step 4)"). The three verification handlers moved behind a `VerificationHandlerContext` shape in [`app/src/lib/sandbox-verification-handlers.ts`](../../app/src/lib/sandbox-verification-handlers.ts) (~580 lines); `executeSandboxToolCall` stays as the public dispatcher, with each case arm delegating through `buildVerificationContext` (`sandbox-tools.ts:402`). Net diff on the dispatcher: 4,112 → 3,614 lines (498 deletions, 25 insertions). Follow-up `83aaa4c` fixed a cache-clear regression for `sandbox_check_types`; `eb823e4` corrected a doc-comment export list. Characterization tests backfilled 2026-04-17 in `c6dec54` as noted under Step 2. **Next family is git/release per the deferred list below** — `sandbox_diff`, `sandbox_prepare_commit`, `sandbox_push`, `promote_to_github` — still inline in the dispatcher (lines 2196, 2238, 2384, 2552 as of 2026-04-18). The handler-context template from this extraction is the starting shape; git/release has higher blast radius (actual GitHub side effects) and the Step 2 → Step 4 ordering should be respected this time.

   Honest framing retained for the git/release pass: the deferred families have the hazards verification deferred around. Read-only inspection has hidden coupling to file version caches, file ledger state, and edit guards (it looks safe and isn't); git/release has live GitHub side effects. Verification's post-hoc record is encouraging — the extraction shipped cleanly and characterization tests backfilled without surfacing regressions — but one family doesn't validate the pattern for families with worse failure modes.
