...
With Gap 1 and Gap 3 Step 1 now shipped, the live gaps are Gap 2 and Gap 3 Steps 2–5:

- **Gap 3 Step 1 was deliberately built Explorer-free so it could ship without Gap 2.** All planner output is executed as `coder` nodes via the existing `runAssistantLoop`, so the Explorer-entry "defense-in-depth hole" the dependency flagged does not apply. The hole does reapply the moment a future pass converts any planner node to `agent: 'explorer'` — at that point Gap 2 (or at least its Explorer subset) becomes a real prerequisite. Until then, Step 1's dependency on Gap 2 is latent, not live.
- **Gap 3 Step 3 will re-surface the Gap 2 dependency.** Typed context-memory retrieval through `runExplorerForTaskGraph` / `runCoderForTaskGraph` routes through the daemon bindings that today enforce via `READ_ONLY_TOOLS`, not `roleCanUseTool`. Landing Step 3 without Gap 2 would mean adding a second typed-memory code path on top of the parallel enforcement vocabulary, which is the exact drift vector Gap 2 is meant to close. Gap 2 before Gap 3 Step 3 is the correct order.
