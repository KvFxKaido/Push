...
1. **Define a `CorrelationContext` contract.** Field meanings (`runId`, `chatId`, `sessionId`, `delegationId`, `taskGraphId`, `taskId`, `toolCallId`, `surface`), owners, nullable cases, and a hard rule that these fields never alter tool args, prompt text, daemon payloads, or sandbox behavior. This is a docs-and-types step, not a propagation step. Signal: a single shared type that the next two steps can lean on.
...
5. **Decide the `coder-agent.ts` shim question.** Architecture decision, not code: is the 610-line web shim a transition artifact (delete after `pushd` primary) or a permanent boundary (formalize and document)? Output is a one-paragraph addendum to `Web and CLI Runtime Contract.md`.

6. **Harden one runtime invariant at the execution layer.** "Explorer cannot mutate" already exists at `explorer-policy.ts:20` and `tool-registry.ts:372` — adding another hook-level check would be duplicate comfort. The useful new check is **lower-level**: the execution runtime itself should reject mutating tools when role is `explorer`, independent of prompt wording, independent of whether the policy hook was registered, and independent of whether the registry was correctly built for the call site. This is what converts "policy-shaped" into "runtime-hard." Signal: a regression test that disables the policy hook and the read-only registry both still gets a refusal from the runtime.
...
- Is anyone using `pushd` as the primary transport yet, or is it still scaffold-grade in real use? The shim decision in step 5 depends on this.
...
Codex noticed during code reading that `sandbox_check_types` near line 2607 of `app/src/lib/sandbox-tools.ts` checks for `tsconfig.app.json` and `tsconfig.node.json` *after* an `ls` command that does not list those files. This is a latent bug in the very code we are about to refactor — exactly the kind of behavior characterization tests should expose before extraction, not after. Worth fixing as a standalone PR before step 4 begins, regardless of whether the larger refactor proceeds.
