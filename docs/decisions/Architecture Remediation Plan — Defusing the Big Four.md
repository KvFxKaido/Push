...
**Current state:** Step 6 added `ROLE_CAPABILITY_DENIED` refusal at `WebToolExecutionRuntime.execute()` — any tool a declared role cannot use is rejected before hooks, approval gates, and Protect Main run. The CLI/pushd path has no equivalent; pushd's Explorer entry points (`cli/pushd.ts:2144/2159/2311/2338`) rely only on policy hooks and registry construction. That is enough defense in depth for the web surface, which today owns Explorer end-to-end; it is *not* enough once Gap 3's task-graph-driven Explorer nodes start running on CLI with smaller models that may hallucinate past registry-level guards.

**Shape of the work:**
- Add a daemon-side `ToolExecutionRuntime` implementation.
- Opt the Explorer entry points in first, matching the Web phasing.
- Capability-grant audits for Coder / Deep Reviewer / Auditor before opting them in — same ordering constraint as the web rollout.
