...
Date: 2026-07-05
Status: **Current** — wired on `claude/copilotkit-research-8bblb4` and `codex/issue-1349-workspace-state-client`: shared vocabulary, builders, reducer, git→state mapper, strict wire validators + published JSON Schema; the **web producer adapter** (`useWorkspaceSandboxController.ts`) and its **consumer** (`WorkspaceSessionScreen` forwards the timeline onto the active chat's run-event stream via `appendRunEvent`, real `Protect Main` threaded in); the **CLI daemon producer** (`cli/pushd.ts` + `cli/workspace-state-emitter.ts`, capability-gated on `workspace_state_v1`); and the **daemon-client consumers** (TUI + `push attach`, both advertising the cap and reducing the timeline). Covered by unit + hook + daemon-integration tests. Remaining: a manual end-to-end walk in a live session — not a blocker. Owner: Push web/runtime.
...
- **CLI daemon emitter.** `cli/pushd.ts` emits the same vocabulary from real filesystem/HEAD signals, reusing `gitStatusInfoToWorkspaceState` + `createWorkspaceStateProducer` verbatim (via `cli/workspace-state-emitter.ts`, which is I/O-injectable and unit-tested). A per-session `WorkspaceStateProducer` lives on the session entry, keyed by `sessionId`: a **snapshot** anchors on `start_session`, a **delta** fires at each run's `finally`, and a **resync** snapshot re-anchors a client on `attach_session`. Emission is **capability-gated** on `workspace_state_v1` (`lib/daemon-capabilities.ts`): the events are live-only and reuse the current `eventSeq`, so pushing them to a client that reconciles the seq-based replay stream without consuming them would corrupt that contract — gating keeps every existing client (and the whole replay-exactness test suite) untouched. A client opts in by advertising the cap in `attach_session.capabilities`.

- **Daemon-client consumers.** `TUI_DAEMON_CAPABILITIES` and `ATTACH_CLIENT_CAPABILITIES` now advertise `workspace_state_v1`. The full-screen TUI folds `workspace.state_snapshot` / `workspace.state_delta` through `reduceWorkspaceStateEvent` into an ambient status-bar view (active branch, dirty count, tracking, Protect Main, sandbox readiness), without appending workspace churn to the transcript. `push attach` uses the same reducer + formatter for compact `[workspace]` lines. Both paths ignore dropped delta outcomes until the next snapshot, and both keep live-only workspace events from advancing the durable replay cursor.

**Staged (next increment):**
- **Live end-to-end walk.** Run a real session (web sandbox and/or daemon) and confirm the snapshot/delta events show up across an edit → commit → branch-switch loop.
