...
Date: 2026-07-05
Status: **Current** — wired on `claude/copilotkit-research-8bblb4`: shared vocabulary, builders, reducer, git→state mapper, strict wire validators + published JSON Schema; the **web producer adapter** (`useWorkspaceSandboxController.ts`) and its **consumer** (`WorkspaceSessionScreen` forwards the timeline onto the active chat's run-event stream via `appendRunEvent`, real `Protect Main` threaded in); and the **CLI daemon producer** (`cli/pushd.ts` + `cli/workspace-state-emitter.ts`, capability-gated on `workspace_state_v1`). Covered by unit + hook + daemon-integration tests. Remaining: a manual end-to-end walk in a live session, and a daemon-client consumer that advertises the cap ([#1349](https://github.com/KvFxKaido/Push/issues/1349)) — neither a blocker. Owner: Push web/runtime.
...
- **Daemon-client consumer.** No TUI/daemon-attached client advertises `workspace_state_v1` or reduces the timeline yet — the daemon produces into a capability-gated wire that currently has no subscriber (the same producer-before-consumer sequencing the web side used). Wiring a client that advertises the cap and folds events through `reduceWorkspaceStateEvent` is the next slice.
