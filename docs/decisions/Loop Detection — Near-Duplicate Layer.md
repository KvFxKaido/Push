...
- **All three loops enforce**: `cli/engine.ts`, `app/src/hooks/chat-send.ts` (via `checkLoopBreaker`, which now returns the `LoopVerdict` and advances ladder state), and the autonomous **Coder** loop (`lib/coder-agent.ts`, newly wired with its own detector + counters — the highest-value site, previously had no near-duplicate coverage at all). Each surface keeps per-run `blocksIssued`/`compactsIssued` and threads them into the oracle.
