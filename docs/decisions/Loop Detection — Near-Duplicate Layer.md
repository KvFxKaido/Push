...
- Telemetry export/persistence: `getLoopMetrics` is in-memory per-process (per-tab on web). Surfacing it (a CLI `/status`-style readout, run-end summary, or the Analytics Engine dataset from `Provider Observability via Analytics Engine.md`) is the step that makes the dark data collectible across runs.
