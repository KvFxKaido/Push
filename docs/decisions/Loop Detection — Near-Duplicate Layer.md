...
Status: Current — graded enforcement landed behind `PUSH_LOOP_DETECTION` (opt-in); near-duplicate ladder dark by default. Prototype added 2026-05-25, graded enforcement 2026-05-29.
...
- `lib/loop-detection.test.ts` — 43 unit tests (tokenization churn, Jaccard edges, per-path isolation, window expiry, streak/threshold boundaries, the full none→warn→block→compact→abort ladder incl. post-compact abort escalation, the web `exactBreakers` shape, dark-vs-enforced gating, and `buildLoopSteeringText` copy per level).
...
- Verdict telemetry: `lib/loop-metrics.ts` (shared, in-memory, per-scope with aggregate fallback — mirrors the `cli/edit-metrics.ts` idiom but lives in `lib/` since both surfaces feed it). Both surfaces call `recordLoopVerdict` every turn with `{ surface, scope, round, level, action, enforced, similarity, reasons }`. The key measurement field is `darkSuppressed` — verdicts where the ladder *would* have fired (level ∈ warn/block/compact/abort) but was suppressed because dark — plus a bounded `recent` buffer to eyeball whether those were real loops. Read via `getLoopMetrics(scope?)`. 11 unit tests in `lib/loop-metrics.test.ts`.
- CLI export/surfacing: `cli/loop-metrics-store.ts` appends a per-run JSONL summary to `~/.push/loop-metrics/verdicts.jsonl` (`PUSH_LOOP_METRICS_DIR` override), and `runAssistantLoop` logs a one-line summary at run end when the run flagged any non-`none` verdict. Each `runAssistantLoop` invocation mints a **unique per-invocation metrics scope** (not `sessionId`/`runId`, which delegation nodes share) and a thin wrapper exports+resets that scope in `finally` — so every run, delegated sub-run, and parallel node accounts in isolation across all exit paths (including the loop-abort). Records carry `sessionId`/`runId`/`outcome`/`rounds` for offline grouping. 4 tests in `cli/tests/loop-metrics-store.test.mjs`.

Graded enforcement landed (2026-05-29), still **dark by default** — the near-duplicate ladder only acts under `PUSH_LOOP_DETECTION=1`; with the flag unset, similarity verdicts resolve to `action: 'none'` and only the always-on exact-match `abort` fires (unchanged behavior). What landed:

- **Shared steering vocabulary**: `buildLoopSteeringText(verdict)` in `lib/loop-detection.ts` is the single source of the `[LOOP_DETECTED]` / `[LOOP_BLOCKED]` / `[LOOP_COMPACT]` copy. All three surfaces consume it; none re-inline the tags (pinned by `cli/tests/loop-detection-drift.test.mjs`).
- **Full ladder in the oracle**: `evaluateLoopState` now escalates similarity `warn → block → compact → abort` via two run-level counters (`blocksIssued`, `compactsIssued`). `compact` is the third block; a further block-strength signal *after* a compaction (`compactsIssued >= 1`) escalates to `abort` ("loop persists after a compact").
- **All three loops enforce the near-duplicate ladder**: `cli/engine.ts`, `app/src/hooks/chat-send.ts` (via `checkLoopBreaker`, which now returns the `LoopVerdict` and advances ladder state; consumption extracted to the `handleLoopVerdict` sibling helper in `chat-send-helpers.ts` to respect the `max-lines` guard), and the autonomous **Coder** loop (`lib/coder-agent.ts`, newly wired with its own detector + counters — the highest-value site, previously had no near-duplicate coverage at all). Each surface keeps per-run `blocksIssued`/`compactsIssued` and threads them into the oracle.
  - **Coder scope**: the Coder takes the **similarity (near-duplicate) ladder only**, not the always-on exact-repeat abort the CLI/web round loops carry. It keeps its existing `mutationFailures` hard-failure breaker and the orchestrator's delegation circuit breaker. Rationale: the Coder legitimately re-reads the same files across many rounds, and an always-on exact-batch abort would cut those runs short (caught by `coder-agent.test.ts`'s checkpoint-cadence fixture). With `PUSH_LOOP_DETECTION` unset every Coder verdict is `none` — purely dark.
- **`coder` metrics surface**: `LoopVerdictSample.surface` gained `'coder'`; the Coder loop calls `recordLoopVerdict` and emits a `coder_loop_verdict` structured log on non-`none` verdicts.

Two deliberate refinements vs. the original draft design below:

- **`warn` skips, it does not execute-then-nudge.** All enforced non-abort levels (warn/block/compact) skip the turn's tool batch and inject the steering note, rather than executing the (N+1)th near-duplicate write. Rationale: once the ladder fires the model has already written N near-identical versions; executing one more is the waste we're stopping. Escalation still advances because `observeWrite` runs during verdict computation regardless of whether the batch executes. This also keeps the CLI and web flows symmetric (inline execute-then-nudge is error-prone in the web round loop's message plumbing). Exposure is bounded — it's all behind the opt-in flag.
- **`compact` resets the detector windows; it does not force a *context* compaction.** No surface exposes a force-compaction gate (the CLI runs `manageContext` every round threshold-driven, web is threshold-driven, the Coder loop has no compaction at all), so wiring a forced context compaction would mean changing shared `trimContext`/`manageContext` signatures across three surfaces — out of scope for the loop-detection ladder. `compact` clears the per-path similarity windows (a real "clear the loop state for a clean restart") and uses the strongest steering copy; forcing an actual context compaction remains a follow-up.

Pending (follow-ups, not yet wired):

- Forced *context* compaction on the `compact` rung (needs a force-gate added to the context pipeline; see refinement note above).
- Web telemetry export: the CLI persists to JSONL (done above), but web verdicts are still in-memory per-tab. Browser verdicts need a POST endpoint to reach a durable sink (the Analytics Engine dataset from `Provider Observability via Analytics Engine.md`) — deferred as its own piece.
