...
Date: 2026-04-12
Status: **Complete** — Phases 1-5 are shipped/closed for the durable scope. Phase 6 is done as of 2026-04-14: `submit_task_graph` executes graphs end-to-end through `lib/task-graph.executeTaskGraph` with `task_graph.*` events on the wire (2026-04-13); crash recovery injects `[DELEGATION_INTERRUPTED]` reconciliation notes; explorer, reviewer, and coder delegation all run through their lib kernels; **real daemon-side Coder AND Explorer tool executors landed 2026-04-14** — `makeDaemonCoderToolExec` wires `executeToolCall` from `cli/tools.ts` with approval gating via `buildApprovalFn`, and `makeDaemonExplorerToolExec` runs the same production tool surface enforcing `READ_ONLY_TOOLS` with no approval gating (Explorer is read-only by design). Both `handleDelegateExplorer` + `runExplorerForTaskGraph` and `handleDelegateCoder` + `runCoderForTaskGraph` produce `'complete'` outcomes on clean kernel returns. v1 synthetic-downgrade shipped 2026-04-13 so v1 clients attached to a v2 daemon see delegation events as `[Role]`-prefixed `assistant_token` shadows on the parent runId. **`multi_agent` is now advertised in CAPABILITIES.** Phase 5E decided Orchestrator stays Web-side; prompt-builder and context-manager extraction landed as optional pushd reuse helpers in `lib/orchestrator-prompt-builder.ts` + `lib/message-context-manager.ts`.
Owner: ishaw
...
1. **Daemon wiring for multi-agent delegation.** SHIPPED 2026-04-14. `pushd` hosts Explorer, Coder, and Reviewer delegation plus task-graph execution through `lib/task-graph.executeTaskGraph` (2026-04-13). Both `makeDaemonCoderToolExec` and `makeDaemonExplorerToolExec` in `cli/pushd.ts` wrap `executeToolCall` from `cli/tools.ts` — Coder gets the full tool surface with approval gating via `buildApprovalFn` (high-risk exec blocks on `submit_approval`), Explorer enforces `READ_ONLY_TOOLS` and runs without approval gating (read-only contract; mutating tool calls get a polite denial resultText). `wrapCliDetectAllToolCalls` / `wrapCliDetectAnyToolCall` transform the CLI's flat detector output into the lib kernels' nested `{ call: { tool, args } }` shape. Both direct RPC handlers (`handleDelegateCoder`, `handleDelegateExplorer`) and task-graph executors (`runCoderForTaskGraph`, `runExplorerForTaskGraph`) produce `status: 'complete'` outcomes on clean kernel returns with empty `missingRequirements`. The full `task_graph.*` event lifecycle continues to flow through `broadcastEvent()` and persist to the session event log. `multi_agent` capability advertised. **Tool-protocol namespace note:** the Explorer kernel's default `EXPLORER_TOOL_PROTOCOL` advertises web-side public tool names (`read` / `repo_read` / `search`) that the daemon's detector does not recognize, so both daemon Explorer call sites pass `sandboxToolProtocol: READ_ONLY_TOOL_PROTOCOL` (exported from `cli/tools.ts`) to override the default with the CLI-named read-only subset (`read_file` / `list_dir` / `search_files` / …). A `cli/tests/daemon-integration.test.mjs` regression test pins `READ_ONLY_TOOL_PROTOCOL` and the `READ_ONLY_TOOLS` set in sync so adding a new read-only tool without updating the prompt fails loudly. Coder's equivalent override is `sandboxToolProtocol: TOOL_PROTOCOL` — same pattern, full tool surface.
...
4. **Approval callback wiring into the daemon.** SHIPPED 2026-04-14 — `makeDaemonCoderToolExec` bakes `buildApprovalFn(sessionId, entry, runId)` into the Coder tool executor closure, so any high-risk exec flagged by `cli/tools.ts:isHighRiskCommand` emits an `approval_required` event on `childRunId` and blocks on a `submit_approval` RPC. Explorer approval binding lands with its real tool executor.
5. **Optional pushd reuse helpers.** Orchestrator stays Web-side by design. The two Phase 5E follow-up extractions have landed: `lib/orchestrator-prompt-builder.ts` and `lib/message-context-manager.ts` now host the pure prompt-builder helpers and a generic context-manager factory, with Web shims at `app/src/lib/orchestrator-prompt-builder.ts` and `app/src/lib/message-context-manager.ts` binding to `ChatMessage`. pushd does not consume them yet — that's a downstream concern for the Coder tool executor work — but the helpers are now a single import path away. Neither extraction blocks daemon wiring.
6. **v1 client synthetic downgrade.** SHIPPED 2026-04-13. Option C from the Wire Format section is now live in `cli/v1-downgrade.ts` + `cli/pushd.ts:broadcastEvent`. `attach_session` accepts an optional `capabilities: string[]` field; clients that include `'event_v2'` receive raw `subagent.*` / `task_graph.*` envelopes, and clients that omit it (including stock v1 clients that never send the field) receive `assistant_token` shadows addressed to the parent runId and prefixed with `[Role]`. `broadcastEvent` fans out per-client using a `sessionClients` map keyed on `{emit, capabilities}`, so a mixed fleet of v1 and v2 clients on the same session each see the appropriate stream. Non-delegation events (`assistant_token`, `tool_call`, `tool_result`, `status`, `run_complete`, `error`, `session_started`, `approval_required`, etc.) pass through unchanged to both classes. Approval events from sub-agents are deliberately passed through verbatim to v1 clients — the daemon-side approvalId → delegation map already routes responses back to the right child, and synthesizing approval events would force v1 clients to speak a sub-agent dialect. `event_v2` is now advertised in the `hello` response CAPABILITIES list so v2 clients can opt in.
...
Shipped 2026-04-14 in `cli/protocol-schema.ts`, then moved to `lib/protocol-schema.ts` during the Remote Sessions work once the web Local PC / Remote adapters began consuming daemon envelopes directly. The module is the canonical runtime validator for the envelope contract above and for the per-type payload shapes of delegation events in the `RunEventInput` union (`subagent.started`/`completed`/`failed` + six `task_graph.*` variants).
...
- **Drift guard-rails** in `cli/tests/protocol-schema.test.mjs` and `cli/tests/protocol-drift.test.mjs` re-extract the delegation event literals, `RunEventSubagent` roles, and `TaskGraphNode.agent` values from `lib/runtime-contract.ts` source text at test time. Adding a new variant to any of those types without a matching entry in `lib/protocol-schema.ts` fails the test with a clear drift message.

### Capability Negotiation (v2 additions)

Current capabilities in `cli/pushd.ts` (grep for `CAPABILITIES`) now advertise: `stream_tokens`, `approvals`, `replay_attach`, `multi_client`, `crash_recovery`, `role_routing`, `delegation_explorer_v1`, `delegation_reviewer_v1`, `delegation_coder_v1`, `task_graph_v1`, **`event_v2`**, **`multi_agent`**.

v2 clients opt into raw delegation events by including `'event_v2'` in the `capabilities` array on any of `attach_session`, `start_session`, or `send_user_message`. Clients that omit the field entirely (or pass an empty array) are classified v1 and receive synthesized `assistant_token` shadows for every `subagent.*` / `task_graph.*` event — see `cli/v1-downgrade.ts` and the "v1 Client Handling" section below. Capabilities pin to the socket on the **first** request that carries them, and subsequent requests with different capability arrays are ignored so delegation-event routing can't flip mid-session. The same capability set also drives `handleAttachSession`'s replay path, so a v1 client that reconnects with `lastSeenSeq` sees synthesized shadows for missed delegation events rather than raw envelopes it can't parse.
...
- `delegation_explorer_v1` / `delegation_reviewer_v1` / `delegation_coder_v1` — RPC paths for direct single-agent delegation. Coder runs through `runCoderAgent` with `makeDaemonCoderToolExec` (full tool surface + approval gating via `buildApprovalFn`). Explorer runs through `runExplorerAgent` with `makeDaemonExplorerToolExec` (read-only contract enforced via `READ_ONLY_TOOLS`; mutating tool calls receive a polite denial resultText). Both actually read the workspace (Coder also writes and runs shell commands). Reviewer is the single-turn JSON review path (no tool loop).
- `task_graph_v1` — `submit_task_graph` accepts dependency-ordered task graphs, executes them via `lib/task-graph.executeTaskGraph`, and streams the full `task_graph.*` RunEvent lifecycle through `broadcastEvent()`. Both `agent: 'coder'` and `agent: 'explorer'` nodes route through their respective real daemon tool executors via `runCoderForTaskGraph` / `runExplorerForTaskGraph`.
- `multi_agent` — advertised 2026-04-14 once both Explorer and Coder daemon-side tool executors were real. The daemon can host end-to-end multi-agent flows (direct delegation RPCs + dependency-ordered task graphs) against the production tool surface.
