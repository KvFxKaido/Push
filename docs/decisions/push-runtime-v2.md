...
Current capabilities at `cli/pushd.ts:54-72` now advertise: `stream_tokens`, `approvals`, `replay_attach`, `multi_client`, `crash_recovery`, `role_routing`, `delegation_explorer_v1`, `delegation_reviewer_v1`, `delegation_coder_v1`, `task_graph_v1`, **`event_v2`**.

v2 clients opt into raw delegation events by including `'event_v2'` in the `capabilities` array on the `attach_session` request. Clients that omit the field (or pass an empty array) are classified v1 and receive synthesized `assistant_token` shadows for every `subagent.*` / `task_graph.*` event — see `cli/v1-downgrade.ts` and the "v1 Client Handling" section below. Capability is pinned to the socket on first attach so auto-attach paths (`start_session`, `send_user_message`) inherit the value.
