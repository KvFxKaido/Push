...
Date: 2026-04-12
Status: **Near complete** — Phases 1-5 are shipped/closed for the durable scope. Phase 6 is effectively done as of 2026-04-14: `submit_task_graph` executes graphs end-to-end through `lib/task-graph.executeTaskGraph` with `task_graph.*` events on the wire (2026-04-13); crash recovery injects `[DELEGATION_INTERRUPTED]` reconciliation notes; explorer, reviewer, and coder delegation all run through their lib kernels; **real daemon-side Coder tool executor landed 2026-04-14** — `handleDelegateCoder` and the task-graph coder path both run through `makeDaemonCoderToolExec` with `executeToolCall` from `cli/tools.ts` + approval gating via `buildApprovalFn`; v1 synthetic-downgrade shipped 2026-04-13 so v1 clients attached to a v2 daemon see delegation events as `[Role]`-prefixed `assistant_token` shadows on the parent runId. The remaining Phase 6 gate for advertising `multi_agent` is the real Explorer tool executor (still scaffold-level; wiring is a copy of the Coder work adapted to the narrower Explorer shape). Phase 5E decided Orchestrator stays Web-side; prompt-builder and context-manager extraction are optional pushd reuse follow-ups, not blockers.
Owner: ishaw
...
1. **Daemon wiring for multi-agent delegation.** `pushd` hosts Explorer, Coder, and Reviewer delegation plus (2026-04-13) task-graph execution through `lib/task-graph.executeTaskGraph`. **Real Coder tool executor landed 2026-04-14** — `handleDelegateCoder` and `runCoderForTaskGraph` both run through `makeDaemonCoderToolExec` in `cli/pushd.ts`, which wraps `executeToolCall` from `cli/tools.ts` with approval gating routed through `buildApprovalFn`. Coder delegations now actually read/write files and run shell commands (under approval for high-risk exec), not a stubbed tool surface. `wrapCliDetectAllToolCalls` transforms the CLI's flat detector output into the lib kernel's `DetectedToolCalls<TCall>` shape, using `READ_ONLY_TOOLS` from `cli/tools.ts` as the source of truth for classification. Outcomes now land as `status: 'complete'` on a clean kernel return instead of the previous `'inconclusive'`. The `task_graph.task_ready` / `task_started` / `task_completed` / `task_failed` / `task_cancelled` / `graph_completed` events continue to flow through `broadcastEvent()` and persist to the session event log. **Still missing:** the real Explorer tool executor (still scaffold-level — same stubbed `toolExec` pattern). Wiring it is a copy of the coder work adapted to the narrower explorer shape (`{ resultText, card? }` vs `{ kind: 'executed'|'denied', ... }`).
...
Current capabilities in `cli/pushd.ts` (grep for `CAPABILITIES`) now advertise: `stream_tokens`, `approvals`, `replay_attach`, `multi_client`, `crash_recovery`, `role_routing`, `delegation_explorer_v1`, `delegation_reviewer_v1`, `delegation_coder_v1`, `task_graph_v1`, **`event_v2`**.
...
- `multi_agent` — advertised once the Explorer tool executor is real (same `makeDaemonCoderToolExec` pattern adapted to explorer's `{ resultText, card? }` shape). Coder's real tool executor landed 2026-04-14 via `makeDaemonCoderToolExec` routing through `executeToolCall` from `cli/tools.ts`, so coder delegations and task-graph coder nodes produce `'complete'` outcomes on clean kernel returns. Explorer is the remaining gate.
...
- `delegation_explorer_v1` / `delegation_reviewer_v1` / `delegation_coder_v1` — RPC paths for direct single-agent delegation. Coder runs through `runCoderAgent` with a REAL daemon tool executor (`makeDaemonCoderToolExec`) backed by `executeToolCall` from `cli/tools.ts`; it actually reads/writes files and runs shell commands under approval gating. Reviewer is the single-turn JSON review path (no tool loop). Explorer is the last scaffold-level path awaiting a real tool executor.
- `task_graph_v1` — `submit_task_graph` accepts dependency-ordered task graphs, executes them via `lib/task-graph.executeTaskGraph`, and streams the full `task_graph.*` RunEvent lifecycle through `broadcastEvent()`. `agent: 'coder'` nodes now route through the real daemon tool executor; `agent: 'explorer'` nodes still use the scaffold helper.
