...
12. Unify runtime intervention machinery (§15 / #1260): the shared steer/block contract, grouping-ledger snapshot, tool-budget block, and tool-call-recovery steer have landed. Phase 1 of the [`Runtime Unification Plan`](../runbooks/Runtime%20Unification%20Plan.md) is complete: web inline, CLI lead, daemon delegations, and Worker background/adoption Coder hosts now use the same stateful policy factory, with a direct-kernel adapter for hosts outside `coder-agent-bindings`. The policy stays state-identical while accepting one explicit host mode at the conversational boundary: delegated Coders apply strict short-response completion grounding, whereas web/CLI leads challenge only explicit completion claims. Announced-action recovery intentionally runs first so a promised tool call can be forced on the next round. Next: execution-ledger updates and the remaining loop/Auditor consumers, without changing agent capability.
