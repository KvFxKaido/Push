...
Status: **Current** (shipped through Phase 4 on 2026-07-17; implementation
provenance in the
...
single trailing side-effect classifier and caps. The ledger sits beside the
turn/run execution path and records what happened: emitted calls, accepted calls,
...
decisions, and Auditor context query that ledger
...
12. Unify runtime intervention machinery (§15 / #1260) — **shipped 2026-07-17**. All four phases of the [`Runtime Unification Plan`](../runbooks/Runtime%20Unification%20Plan.md) are complete: every Coder host uses the shared stateful policy factory; copy-shaped provider transports converge by wire family; Anthropic `pause_turn` continuation is one cross-shell state machine; web/CLI capability profiles use one resolver with surface-provided metadata; and the shared Coder kernel records accepted/rejected calls plus execution outcomes in a run-scoped ledger consumed by loop and Auditor policy. Reasoning-call recovery, loop decisions, incomplete evaluation, and unsafe/unavailable delivery gates now use typed runtime interventions. Malformed-call reduction is shared while storage stays shell-local, and CLI compaction activates the user-owned goal anchor. Completion grounding remains selected by turn intent rather than host, provider quirks remain explicit at leaf adapters, and this work adds no agent capability or generic hook framework.
