...
12. Unify runtime intervention machinery (§15 / #1260): the shared steer/block contract, grouping-ledger snapshot, tool-budget block, and tool-call-recovery steer have landed. Phases 1–3 of the [`Runtime Unification Plan`](../runbooks/Runtime%20Unification%20Plan.md) are complete: every Coder host uses the shared stateful policy factory, copy-shaped provider transports converge by wire family, Anthropic `pause_turn` continuation is one cross-shell state machine, and web/CLI capability profiles use one resolver with surface-provided metadata. Completion grounding remains selected by turn intent rather than host, and provider quirks remain explicit at leaf adapters rather than disappearing into a mega-adapter. Next: execution-ledger updates and the remaining loop/Auditor consumers, without changing agent capability.
