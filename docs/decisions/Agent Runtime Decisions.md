...
12. Unify runtime intervention machinery (§15 / #1260): the shared steer/block contract, grouping-ledger snapshot, tool-budget block, and tool-call-recovery steer have landed. Phase 1 of the [`Runtime Unification Plan`](../runbooks/Runtime%20Unification%20Plan.md) is complete: web inline, CLI lead, daemon delegations, and Worker background/adoption Coder hosts now use the same stateful policy factory, with a direct-kernel adapter for hosts outside `coder-agent-bindings`. Completion grounding is selected by turn intent rather than host: task-shaped turns stay strict on every surface, while explicitly conversational lead turns use claim-only grounding. Announced-action recovery intentionally runs first so a promised tool call can be forced on the next round, while a round containing a structured native call bypasses text-only policy because the action is already present. Next: execution-ledger updates and the remaining loop/Auditor consumers, without changing agent capability.
