...
(`/api/auth-probe`) returns `authed` — no exceptions — and the session is
minted solely by the App-OAuth path (which also returns the token), so passing
the gate ⟹ you have a session ⟹ you went through OAuth ⟹ you have `authToken`.
Identity and token are coupled.
...
2. `App.tsx`: remove the screen-logic short-circuits + comments premised on
   "signed out of GitHub" (689–708), and the `authToken`-optional branches that
   only mattered for an accountless user. Where a short-circuit also encodes
...
3. Audit for hardcoded GitHub checks that should be probe/session checks; leave
   the session-provider abstraction intact (no new GitHub-specific gating).
4. Tests: a signed-in user still reaches scratch / chat / repo / daemon
   workspaces; no path renders a workspace without a session; the gate remains
   the single entry point.

## Open questions

1. **Offline / loopback local-pc.** Requiring a session means a probe
   (network) — does that break a genuinely-offline loopback daemon session? If
   pure-offline local-pc is a goal, the bearer-as-identity-provider path is how
   it'd be honored later; v1 accepts the network probe.
