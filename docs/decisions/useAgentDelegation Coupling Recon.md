...
The hook is the third-largest architectural hotspot in Push (see `docs/decisions/Architecture Remediation Plan — Defusing the Big Four.md`, Step 5). The plan prescribes splitting it "by role workflow — extract typed-memory helpers first, then Coder/Explorer execution, Auditor evaluation, and task-graph execution into adapter modules." The pattern proven 5 times on `app/src/lib/sandbox-tools.ts` (4,112 → 475 lines at the 2026-04-18 trough, since regrown to 670 under feature pressure — net −84%; the regrowth itself is the Waterbed Effect signal flagged in the Big Four plan's 2026-05-01 update) is: pin behavior with tests at the dispatcher level, extract each handler family behind a `*HandlerContext` shape with a `build*Context` helper, compress the dispatcher's case arm into a one-line delegation, and route all shared state to handlers only through the named context (no module globals, no implicit closure reaches).
