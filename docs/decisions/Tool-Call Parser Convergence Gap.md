...
| **#683** | Replace three workaround heuristics with exact API data: `RecoveredNamespacedCall.endOffset`, `RecoveredXmlCall.endOffset`, `ToolDispatchResult.callOffsets`. Also fixed the `<function_calls>` invoke offset re-anchor bug (Copilot caught) and the recovery-args region shape-vs-claim safety gap (Codex P1 caught — restored the shape-based regex lookback for the legacy-scan skip while keeping the new precise-offset APIs for the textual-order merge). | Closes three heuristics: kernel-call offset reconstruction via `text.indexOf`, recovery-args region detection via 80-char regex lookback, and the `<function_calls>` invoke offset that was undercounting by the opening tag length. All now use exact data from the recovery/dispatcher APIs. |
