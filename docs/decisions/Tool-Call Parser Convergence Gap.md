...
Status: **Resolved 2026-05-28** — both CLI and Web now route through the shared `createToolDispatcher` kernel at `lib/tool-dispatch.ts`. The original CLI side closed 2026-04-15 (missing-fence drop) + 2026-04-18 (four silent-drop modes, PR #334). The Web side closed in a six-PR tranche on 2026-05-28: **#677** extracted phase-grouping to `lib/tool-call-grouping.ts`; **#679** migrated web's `detectAllToolCalls` onto `createToolDispatcher` (textual-order merge, three legacy-fallback shapes); **#680** unified the caps surface (web + CLI both pass `DEFAULT_GROUPING_CAPS`) and split `batchOverflow` from `extraMutations` so callers can emit precise error codes; **#681** added `enableInternalRecovery: false` for callers that run their own gated recovery + closed the recovery-args-region bypass in the legacy fallback; **#683** added new exact-data APIs to the recovery/dispatcher layer (`RecoveredNamespacedCall.endOffset`, `RecoveredXmlCall.endOffset`, `ToolDispatchResult.callOffsets`), replacing two workaround heuristics (kernel-call offset reconstruction via `text.indexOf`, `<function_calls>` invoke offset re-anchor) with exact data. The third heuristic — `isInsideRecoveryArgsRegion`'s 80-char shape-based lookback — was intentionally retained after Codex P1 surfaced a safety gap: the recovery functions reject prose mentions on their trailing-context gate, but the args object still gets picked up by bare-args inference, so the legacy-scan skip needs to be shape-based (not recovery-claimed-based). **#684** brought web's user-facing error surface in line with the kernel split — both `FILE_MUTATION_BATCH_OVERFLOW` and `MULTI_MUTATION_NOT_ALLOWED` are now emitted with distinct codes and hints on both surfaces, and a silent-drop regression where batch overflow could disappear without a model-facing error was closed. See [*Fix Direction for Layer 2*](#fix-direction-for-layer-2) below for the historical narrative. **The two open follow-ups (per-source `ToolSource` split + bare-block-eligibility loosening) have no current forcing function and are tracked as reference only.**
...
The web tranche landed in six PRs, each closing one piece of the shape-mismatch problem this section originally framed:
...
| **#683** | Added exact-data APIs to the recovery/dispatcher layer (`RecoveredNamespacedCall.endOffset`, `RecoveredXmlCall.endOffset`, `ToolDispatchResult.callOffsets`). Replaced two workaround heuristics with exact data: kernel-call offset reconstruction (was using `text.indexOf` to re-derive offsets the kernel actually knew internally) and `<function_calls>` invoke offset re-anchor (was undercounting by the opening tag length — Copilot review caught). The third heuristic — `isInsideRecoveryArgsRegion`'s 80-char shape-based lookback — was intentionally retained after Codex P1 surfaced a safety gap. | Closes two heuristics with exact API data; documents why the third (legacy-scan recovery-args region skip) intentionally stays shape-based: recovery functions REJECT prose mentions on their trailing-context gate, but the args object still gets picked up by `tryRecoverBareToolArgs`, so the skip needs to be shape-based (matches prefix shape regardless of whether recovery would claim it). The new `endOffset` APIs remain available for other callers. |
| **#684** | Web `handleMultipleMutationsError` gains a `batchOverflow` field and emits distinct error codes per category. `chat-send.ts` fires the error on `batchOverflow.length > 0` too — closing a silent-drop regression where 9+ file mutations could execute 8 and lose the rest with no model-facing feedback (introduced by the kernel split in #680). | Web's user-facing error surface now matches CLI's per-code distinction. `mutating` is no longer mislabeled as a `MULTI_MUTATION_NOT_ALLOWED` violation when only batch overflow triggered the rejection (Codex P2 / Copilot regression fix). |
...
- The cap surface and `batchOverflow` / `extraMutations` split documented in `lib/tool-call-grouping.ts` is the canonical contract. Both CLI (`cli/engine.ts`) and web (`app/src/lib/chat-tool-execution.ts:handleMultipleMutationsError`) consume the two lists with matching structured error codes: `FILE_MUTATION_BATCH_OVERFLOW` for batch overflow, `MULTI_MUTATION_NOT_ALLOWED` for ordering violations. Surfaces should NEVER re-derive the overflow condition from a global flag (was a bug on both surfaces — see PR #680 Copilot review and PR #684 Codex P2).
