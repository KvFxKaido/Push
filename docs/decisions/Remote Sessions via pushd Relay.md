...
| 2.e — pushd outbound dial + reconnect | `cli/pushd-relay-client.ts` opens an outbound WS to the Worker; reuses the Phase 1.f reconnect ladder `[1s, 2s, 4s, 8s, 16s, 30s]` capped at 5 attempts. `push daemon relay enable\|disable\|status` CLI surface. Audit events for `relay.connect` / `relay.disconnect` via the Phase 3 slice 3 audit log. Origin-gate carve-out for pushd (no browser Origin header). pushd emits `relay_phone_allow` / `relay_phone_revoke` envelopes when phones are paired / unpaired. New `~/.push/run/pushd.relay.json` (chmod 0600) holds the deployment-scoped relay token. | Open | — |
