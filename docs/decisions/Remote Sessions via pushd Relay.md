...
Status: **Phase 1 + Phase 3 shipped; Phase 2 storage decided (2.a)** — Phase 1.a–1.f (#507–#511, 3c.2a/b, 3c.3, auto-reconnect 2026-05-13) and Phase 3 slices 1–4 (#518–#521) landed. Phase 2 storage shape locked 2026-05-13: DO in-memory ring buffer, no durable storage (see Q#2 + Phase 2 sub-phase table). Remaining: Phase 2.b–2.f (Worker/DO relay code), Phase 4 (desktop wrapper).
Owner: Push
...
| 2 — Worker-mediated relay | Outbound-from-PC WebSocket to a Worker/DO that pairs phone client to daemon. Storage shape decided 2026-05-13 (DO in-memory ring buffer, Q#2 below); sub-phase breakdown drafted in the Phase 2 sub-phase table below. | In progress (2.a doc lock shipped) | claude/phase-2-relay-storage-TfQfR |
| 3 — Permission + audit model | Repo allowlist, per-session attach token, connected-device list, audit log with surface/device provenance. | **Core shipped** — slice 1 (allowlist + connected-device list + live revoke) via #518; slice 2 (device-attach token + cascade) via #519; slice 3 (NDJSON audit log with size-rotation + structural-only privacy default) via #520; slice 4 (submit_approval UI + `approval.decision` audit event) on `claude/remote-sessions-phase3-submit-approval`. All four minimum-model items shipped. | #518 · #519 · #520 · claude/remote-sessions-phase3-submit-approval |
...
| 2.b — Relay DO scaffold | New `RelaySessionDO` class with WebSocket Hibernation API; wrangler `RELAY_SESSIONS` DO binding; `/api/relay/v1/...` route in `worker.ts`. Accepts WS connections; no protocol forwarding or auth yet. Unit-tested via the worker test harness. | Open | — |
| 2.c — Two-sided auth + pairing | pushd authenticates with a deployment-scoped relay token (new `~/.push/run/pushd.relay.json`, chmod 0600); phone authenticates with a Phase-3-style attach token (`pushd_da_*`). DO pairs them under a sessionId derived from the attach token's parent device. Reuses `lib/protocol-schema.ts` envelopes — no new vocabulary. | Open | — |
| 2.d — DO in-memory ring buffer + replay | Per-DO `Map<seq, envelope>` capped by both count (default `N=256`) and age (default `Y=60s`); first-to-fire evicts. Phone reconnect with `Last-Event-Seq` header replays the tail; a gap larger than the buffer emits `replay_unavailable` so the client falls back to `attach_session` for current state rather than silently missing envelopes. Defaults override-able by env (`PUSH_RELAY_BUFFER_COUNT`, `PUSH_RELAY_BUFFER_AGE_MS`). | Open | — |
| 2.e — pushd outbound dial + reconnect | `cli/pushd-relay-client.ts` opens an outbound WS to the Worker; reuses the Phase 1.f reconnect ladder `[1s, 2s, 4s, 8s, 16s, 30s]` capped at 5 attempts. `push daemon relay enable\|disable\|status` CLI surface. Audit events for `relay.connect` / `relay.disconnect` via the Phase 3 slice 3 audit log. | Open | — |
...
Each future sub-phase gets its own scope-confirmation before code lands. The breakdown is PR-per-row; 2.b→2.f must land in order because each depends on the previous (scaffold → auth → buffer → daemon dial → client mode).
...
2. ~~Relay storage: Durable Object memory only, short event log in DO storage, KV/D1 index, or no durable replay in v1?~~ **Answered (2026-05-13, Phase 2 decision lock — 2.a)**: DO in-memory ring buffer per session; no durable storage. A Cloudflare Durable Object with an active WebSocket stays alive, so as long as `pushd`'s outbound WS is connected the DO instance (and its in-memory state) persists — durable storage would only matter across DO eviction, which co-occurs with pushd disconnect anyway, at which point there is no live source emitting events to replay. The buffer is bounded by both count (default `N=256` envelopes) and age (default `Y=60s`); the first cap to fire evicts. Phone reconnect with `Last-Event-Seq` header replays from the buffer; a gap larger than the buffer emits a `replay_unavailable` event so the client falls back to `attach_session` for current state rather than silently missing envelopes. No envelope content (file paths, command text, model output) lands in durable storage — keeps the "smallest useful event window" literal from the Security Notes. KV/D1 deferred; revisit if Phase 4 desktop wrapper needs cross-session listing or if real-world telemetry shows reattach holes. Implementation tracked in the Phase 2 sub-phase table above (2.d).
