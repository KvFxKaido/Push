...
Status: **Phases 1, 2, and 3 shipped** — Phase 1.a–1.f (#507–#511, 3c.2a/b, 3c.3, auto-reconnect 2026-05-13) and Phase 3 slices 1–4 (#518–#521) landed. Phase 2 storage shape locked 2026-05-13: DO in-memory ring buffer, no durable storage (see Q#2 + Phase 2 sub-phase table). Phase 2.b (#524), 2.c (#525), 2.d.1 (#526), 2.d.2 (#528), 2.e (#529), and 2.f (#530) all landed 2026-05-13. **The relay path is end-to-end testable from a phone surface as of #530 — the milestone the track was built around.** Hash/token-id allowlist hardening landed 2026-05-13 (2.g, #531): the relay wire and DO now allowlist `sha256(bearer)` base64url-encoded instead of bearer plaintext, and pushd reseeds the in-memory registry from the persisted attach-token store at startup, so a daemon restart is invisible to already-paired phones. Long-lived WS reuse for chat dispatch landed 2026-05-13 (2.h below): chat-layer `sandbox_*` tool calls now route through the hook-owned WebSocket via a `LiveDaemonBinding` instead of opening a transient WS per call. Remaining: Phase 4 (desktop wrapper polish), plus follow-up for `DaemonChatBody` extraction.
Owner: Push
