...
Date: 2026-05-07 (Phase 1 substrate + first-tool dispatch shipped 2026-05-11 → 2026-05-12; chat-layer wiring, tool-op fan-out, and UI half landed 2026-05-12)
Status: **Phase 1 substantially shipped** — substrate (#507–#510), dispatch seam (#511), chat-layer runtime wiring (3c.2a), file-op fan-out (3c.3), and UI half + cancel button (3c.2b) all landed. Remaining: auto-reconnect + daemon-side mid-run cancellation (1.f hardening), submit_approval surface (1.f, no current consumer), and Phase 2+ (Worker relay, permission/audit model).
Owner: Push
...
| 1.f — Cancel button (partial) | Stop button in `LocalPcChatScreen` header, conditional on `isStreaming`, calls `abortStream` to halt the in-flight web round. Web-side abort only — the dispatch model is transient (one tool per WS), so any in-flight `sandbox_exec` runs to its own 60s timeout. Documented gap. | Shipped 2026-05-12 (web-side abort) | PR 3c.2b |
| 1.f — Auto-reconnect with backoff | When the long-lived `useLocalDaemon` WS drops, retry the open with exponential backoff so a brief network blip doesn't end the session. Today there's only a manual "Retry" affordance via `reconnect`. | Open | — |
| 1.f — Daemon-side mid-run cancellation | Persistent session/runId abstraction so `cancel_run` (already implemented in pushd) can actually kill an in-flight `sandbox_exec` on the daemon side. Today the transient binding closes on web abort but the child process runs to its own timeout. | Open | — |
