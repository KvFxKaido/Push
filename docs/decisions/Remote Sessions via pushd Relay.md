...
Date: 2026-05-07 (Phase 1 substrate + first-tool dispatch shipped 2026-05-11 → 2026-05-12; chat-layer wiring, tool-op fan-out, and UI half landed 2026-05-12; 1.f hardening landed 2026-05-13)
Status: **Phase 1 shipped (1.a–1.f core)** — substrate (#507–#510), dispatch seam (#511), chat-layer runtime wiring (3c.2a), file-op fan-out (3c.3), UI half + cancel button (3c.2b), and 1.f hardening (auto-reconnect with backoff + daemon-side mid-run cancellation) all landed. Remaining: submit_approval surface (1.f, no current consumer), and Phase 2+ (Worker relay, permission/audit model).
Owner: Push
...
| 3 — Permission + audit model | Repo allowlist, per-session attach token, connected-device list, audit log with surface/device provenance. | **In progress** — slice 1 (allowlist + connected-device list + live revoke) shipped via #518; slice 2 (device-attach token: short-lived bearer minted on pair, replaces durable device token in IndexedDB, cascade-revoked from parent) shipped via #519; slice 3 (NDJSON audit log at `~/.push/run/pushd.audit.log`, size-rotated 10MB×5, structural-only by default with `PUSHD_AUDIT_LOG_COMMANDS=1` opt-in) on `claude/remote-sessions-phase3-audit-log`. Remaining: approval-provenance + submit_approval UI. | #518 (slice 1) · #519 (slice 2) · claude/remote-sessions-phase3-audit-log (slice 3) |
