...
| `Streaming UI Deltas.md` | Draft, scoping-validated 2026-05-01 | Proposal to ship `tool_use_delta` SSE events backed by a generic `pending-tool` placeholder card that swaps for the real typed card on tool completion. Card-type fidelity dropped from the original draft because tools decide cardType from output shape, not at dispatch. Borrowed shape only from the third-party Claudio APK reverse-engineer; sourcing caveat documented in-doc. Estimated half-to-full-day implementation, blocked on binding-layer plumbing in `@push/lib/coder-agent-bindings`. |
