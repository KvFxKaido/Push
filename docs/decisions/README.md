...
| `CorrelationContext Contract.md` | Current, added 2026-04-14 | Step 1 of the remediation plan: the canonical shape for passive cross-surface correlation tags (`runId`, `chatId`, `sessionId`, `taskGraphId`, `taskId`, `executionId`, `toolCallId`, `surface`). Docs-and-types only; nothing imports `lib/correlation-context.ts` yet. |
