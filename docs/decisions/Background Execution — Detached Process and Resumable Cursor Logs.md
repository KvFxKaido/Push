...
**Status:** Draft, added 2026-06-04. Worker routes **+ `SandboxProvider` wiring + a shared `lib/` run-to-completion kernel + first consumer + unit tests** landed on `feat/cf-background-exec`. CF provider implements all four methods behind `capabilities.backgroundExec`; Modal declares it `false`. **First consumer wired:** the cold `npm install` in `handleCheckTypes` now runs through the detached path (via `execLongRunningInSandbox` in `sandbox-client.ts`, driven by `lib/detached-exec-runner.ts`), with transparent fallback to buffered `exec` on backends without the routes. Needs a `ROADMAP.md` entry to graduate. DO-lifetime assumption **verified live** (see Evidence).
