...
Date: 2026-07-13
Status: **Current** — **Slices 0–2, the Slice 3 high-traffic and Git-inspection producer cohorts, Slice 4, and Slice 5 shipped**: the shared `ToolCard` vocabulary lives in `lib/tool-cards.ts`; the kernel carries cards on `tool.execution_complete` without exposing them to the model; inline TUI, daemon-attached TUI, and `push attach` render the same bounded generic fallback — title, key/value rows, and a headed section for list-of-object fields (so a `ci-status` card shows you *which* check failed rather than `Checks: 3 items`). Daemon delivery is gated by `tool_cards_v1`, with cards stripped from legacy clients on both live and replay paths. Command execution, file writes/edits, test/typecheck, Git status/diff/commit/push, and delegation producers now declare cards across the applicable web and CLI paths. Narration splitting now lives in `lib/tool-prose.ts`; the kernel emits ordered, render-only `assistant.tool_prose` events that both web and TUI consume without adding synthetic model messages. Both shells now render from declared structure instead of sniffing text or guessing which argument matters. The Slice 3 catalog tail remains — measured for the GitHub cohort under Slice 3 below, and sorted by render shape rather than by read-only/mutating, because only the list- and scalar-shaped tools are free. Owner: Push runtime (shared `lib/` + both shells).
...
- **Slice 2 — Consumers.** TUI reads `card` off the run event (inline **and** daemon-attached — one producer, so both work for free) and renders via a generic typed fallback: title + key/value rows derived from the card's declared shape, plus a headed section for any field holding a list of objects, summarized from that item's declared scalar fields. The daemon strips cards for clients that don't advertise `tool_cards_v1`. The JSON-dump path stops being reachable for card-bearing tools.
- **Slice 3 — Producers.** Walk the tool catalog and attach cards, highest-traffic first (`sandbox_exec`, file writes/edits, test/typecheck, commit/push, delegation). Each tool that gains a card immediately improves every surface.
  - **Measured tail (2026-07-14), GitHub cohort.** 13 of 30 `execute*Tool` functions in `lib/github-tool-core.ts` declare a card. The 17 that don't are **7 read-only** and **10 mutations** (`create_pr`, `merge_pr`, `trigger_workflow`, the issue writes, ...). The 7 read-only ones are **not one cohort** — sorting them by the shape the renderer would have to handle is what tells you the actual cost:
    - **Object lists — declaring the card is the whole job (4).** `list_issues` and the code-scanning / dependabot / secret-scanning alert tools. Homogeneous lists of objects: exactly what the generic fallback now renders as a headed section. No renderer work.
    - **Scalars with an embedded list — also covered (1).** `check_pr_mergeable`: a scalar headline (`mergeable`, `mergeable_state`) plus a checks list. Rows + a section, both already supported.
    - **Long text bodies — these need a formatter (2).** `get_job_logs` returns a log body; `get_issue` returns a single issue whose `body` is prose. The generic fallback would truncate either to a 180-char row. These need `bodyLines` — the shape `formatDiffPreviewCard` / `formatSandboxStateCard` already use — so they are a specialized formatter, not a free win.

    The general lesson under that split: **"read-only" is not a render shape.** List, scalar, and body are, and only the first two are free. (An earlier revision of this doc called all 7 "list-shaped, no renderer work"; the autonomous reviewer caught it. A card whose payload carries a multi-line string is the one generic shape the fallback still does not handle — promoting multi-line string fields to `bodyLines` would close both, and is the obvious next generalization if a third such tool appears.)

    The sandbox/CLI cohorts are **not** counted here on purpose: their dispatch is spread across `sandbox-tools.ts` → delegated handlers → `coder-agent-bindings.ts` → `scratchpad-tools.ts` → `memory-tool-exec.ts` with several different shapes, and a static scan of it is not trustworthy enough to publish a number from. Count that cohort by hand when you work it.
...
- **Slice 5 — Delete the sniffing. Shipped.** Remove `looksLikeUnifiedDiff`, `looksLikeToolCall`, and the arg-guessing fallback once no path depends on them. **This slice is the acceptance criterion for the whole track:** when nothing in either shell has to guess what it is rendering, the contract is real.
...
- **Slice 3 is long-tail work** across ~68 tools. Mitigated by the generic typed fallback: a tool without a bespoke card still renders structurally, so the tail is an improvement curve rather than a cliff.
  - That mitigation was **only half-true as first shipped**, and the gap is worth recording because the acceptance criterion could not see it. The fallback rendered scalar fields but collapsed any array it could not join to `"N items"` — and a list of objects is the shape of nearly every inspection card, holding the entire reason the tool was called. A `ci-status` card with a failing check rendered as `Checks: 3 items`. So the tail was a cliff after all for exactly the cards that mattered most, while Slice 5's criterion (*the sniffing code is deleted*) read fully green. **A code-shaped acceptance criterion cannot see a presentation-shaped failure.** Fixed by rendering object lists as sections; the lesson is that this track needs a criterion someone has to *look* at.
