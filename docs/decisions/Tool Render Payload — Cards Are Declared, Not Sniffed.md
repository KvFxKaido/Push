...
- **Slice 3 — Producers.** Walk the tool catalog and attach cards, highest-traffic first (`sandbox_exec`, file writes/edits, test/typecheck, commit/push, delegation). Each tool that gains a card immediately improves every surface.
  - **Measured tail (2026-07-14), GitHub cohort.** 13 of 30 `execute*Tool` functions in `lib/github-tool-core.ts` declare a card. The 17 that don't are **7 read-only, list-shaped** (`list_issues`, `get_issue`, `get_job_logs`, `check_pr_mergeable`, and the code-scanning / dependabot / secret-scanning alert tools) and **10 mutations** (`create_pr`, `merge_pr`, `trigger_workflow`, the issue writes, ...). The 7 list-shaped ones are the cheap win: they are the exact shape the generic fallback now renders as a section, so declaring the card is the whole job — no renderer work. The sandbox/CLI cohorts are **not** counted here on purpose: their dispatch is spread across `sandbox-tools.ts` → delegated handlers → `coder-agent-bindings.ts` → `scratchpad-tools.ts` → `memory-tool-exec.ts` with several different shapes, and a static scan of it is not trustworthy enough to publish a number from. Count that cohort by hand when you work it.
