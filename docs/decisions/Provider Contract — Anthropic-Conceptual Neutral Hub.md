...
2. **Capability profiles decide degradation, not scattered provider-name checks.** `resolvePushCapabilityProfile(provider, model, route)` is now the request/stream-path seam for native tool calling, structured output, content blocks, reasoning-block replay, multimodal support, and context tier. Model allowlists and provider metadata still live underneath the resolver where they belong, but callers should read the profile instead of re-deriving those gates locally.
