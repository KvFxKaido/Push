...
2. **Capability profiles decide degradation, not scattered provider-name checks.** `resolvePushCapabilityProfile(provider, model, route)` is now the request/stream-path seam for native tool calling, structured output, OpenAI-family wire transport, content blocks, reasoning-block replay, multimodal support, and context tier. Curated seed evidence and provider metadata still live underneath the resolver where they belong, but callers should read the profile instead of re-deriving those gates locally. OpenRouter is the model-axis transport case: its catalog does not currently advertise `/responses` endpoint support, so verified IDs remain private cold-start evidence behind `openaiWire` until a discoverable field exists.
