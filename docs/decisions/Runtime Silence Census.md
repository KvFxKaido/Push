...
- **Wave 4 — local-first entry (the big one): SCOPED 2026-07-27.** Rows A1, E1, E2, M2. Kills the A1 wait class at the root rather than restyling it. Three PRs, and the order is load-bearing:
  - **4a — Files, both surfaces.** A UI-level read seam behind `useFileBrowser`, so `HubFilesTab` and the `FileBrowser` section inherit it together. Retires E1/E2's refusal copy and Start-workspace button, and closes the native split-brain (finding 1).
  ...
  - **4b — Diff: SHIPPED 2026-07-29.** The production-tab regression first reproduced the unchecked "No working tree changes." claim. `HubDiffTab` now owns a guarded first-visible load: web runs accept-warm-read through Wave 3's `useWarmAction`, while a resolved native clone reads `nativeFs.diff()` without warming cloud infrastructure. Native results pass through the same shared sensitive-file/secret sanitizer as the tool path, and an empty result is retained as checked data so tab re-entry cannot confuse "not loaded" with "clean." The Start-workspace refusal surface and the parent's empty auto-load effect are gone; warm failure keeps the Wave 3 honest-failure copy.
