...
- **Sandbox lifetime vs job lifetime.** `useSandbox.ts` tears down sandboxes on branch switch. If the user switches branch while a background job is running, the sandbox id becomes stale. Phase 1 mitigation: reject `/api/jobs/start` if `repoFullName + branch` don't match a live sandbox; on branch switch in UI, prompt "cancel running job?" or let the next sandbox call 404 and the DO records the failure.
