...
Design doc: `docs/runbooks/Background Coder Tasks Plan.md`
...
- Notes: Track B has significantly reduced payload size and eliminated a major class of edit failures where the model would guess missing code. See `docs/archive/runbooks/Truncation-Aware Edit Safety Plan.md`.
...
| Move long-running orchestration to server-side jobs | defer | Useful in theory, but high complexity for current PWA scope after resumable sessions shipped | Keep `docs/runbooks/Background Coder Tasks Plan.md` as reference; revisit for native app/CLI runtime direction |
...
- `docs/runbooks/Harness Runtime Evolution Plan.md` is now also complete and should be read as follow-up implementation history, not as the active roadmap.
