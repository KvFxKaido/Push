...
export interface TransformableMessage {
  ...
  /** Marks a message as having been generated by the transformer itself
   *  rather than the user/model. Set by the goal-anchor and session-digest
   *  factories. The transformer uses this flag (not content text-shape) to
   *  identify its own synthetic messages when looking up a prior digest for
   *  merge-in-place — text-shape detection is spoofable by user/tool
   *  messages that contain an exact-form digest block (PR #574 review).
   *  Optional so callers that don't surface the flag still work; their
   *  messages just won't be treated as synthetic. */
  synthetic?: boolean;
}
...
/**
 * Mark a transformer-generated synthetic message so downstream stages and
 * detection helpers (`isSyntheticDigestMessage`) can identify it without
 * inspecting content. Caller-supplied factories shouldn't need to know
 * about this flag — the transformer stamps it after the factory returns.
 * Keeps text-shape spoofing closed: even an exact-form digest block in a
 * user/tool message lacks this flag and is correctly classified as
 * non-synthetic.
 */
function stampSynthetic<M extends TransformableMessage>(msg: M): M {
  return { ...msg, synthetic: true };
}
...
function injectUserGoalStage<M extends TransformableMessage>(): Stage<M> {
  return {
    ...
    run: (messages, options, state) => {
      ...
      const goalMessage = stampSynthetic(options.createGoalMessage!(blockContent));
  ...
  return {
    ...
    run: (messages, options, state) => {
      ...
      if (priorIdx !== -1 && priorFromTranscript) {
        ...
        updated[priorIdx] = stampSynthetic(options.createSessionDigestMessage!(newContent));
      ...
      const digestMessage = stampSynthetic(options.createSessionDigestMessage!(blockContent));
      if (messages.length === 0) {
