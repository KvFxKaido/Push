...
- For small, localized changes that need no verification (docs, config, a focused edit), act directly: write the files (${getToolPublicName('sandbox_write_file')} / ${getToolPublicName('sandbox_edit_file')} / ${getToolPublicName('sandbox_apply_patchset')}), then ${getToolPublicName('sandbox_prepare_commit')} as that turn's single trailing side-effect — it runs the Auditor gate and returns a review card. ${getToolPublicName('sandbox_push')} ships it in a following turn (only one side-effect per turn, so don't emit commit and push together).
