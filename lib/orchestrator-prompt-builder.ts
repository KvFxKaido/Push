...
- For small, localized changes that need no verification (docs, config, a focused edit), act directly: ${getToolPublicName('sandbox_write_file')} / ${getToolPublicName('sandbox_edit_file')} / ${getToolPublicName('sandbox_apply_patchset')}, then ${getToolPublicName('sandbox_prepare_commit')} + ${getToolPublicName('sandbox_push')} to ship it. The commit still goes through the Auditor gate.
