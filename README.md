...
Provider keys can also be pasted into Settings at runtime. Azure OpenAI and Bedrock still use base URL + model/deployment setup. Vertex now uses a Google service account JSON plus region and model in the normal path, with the old raw Vertex OpenAPI endpoint flow kept as a legacy fallback. When 2+ provider keys are set, a backend picker appears in Settings. Settings owns the default backend/model picks plus the app's active backend preference, the chat composer owns the current chat selection, delegated Coder runs inherit that chat lock, Reviewer keeps its own sticky provider/model selection, and Auditor still follows the active backend. Web default mode is **Auto** (Zen-first when available). In local development, with no AI keys configured, the app falls back to the demo-provider path; separately, when no GitHub token is configured, repo and PR views fall back to mock data.
