...
Bring your own provider: Ollama Cloud, OpenRouter, OpenCode Zen, Blackbox AI, or Nvidia NIM. Advanced Settings also expose opt-in private connectors for Azure OpenAI, AWS Bedrock, and Google Vertex.
...
For a self-hosted deployment, run the app on Cloudflare Workers. The worker at `app/worker.ts` proxies the built-in chat routes (`/api/ollama/chat`, `/api/openrouter/chat`, `/api/zen/chat`, `/api/blackbox/chat`, `/api/nvidia/chat`), the opt-in private-connector routes (`/api/azure/chat`, `/api/bedrock/chat`, `/api/vertex/chat`), and `/api/sandbox/*` to Modal web endpoints. Built-in provider keys live as runtime secrets; the Vertex route can also mint Google access tokens server-side from the saved service-account JSON and route Gemini through Vertex OpenAPI while translating Claude partner-model responses back into the app's OpenAI-style SSE stream. Static assets are served by the Cloudflare Assets layer. The Modal sandbox backend at `sandbox/app.py` is deployed separately via `modal deploy`.
