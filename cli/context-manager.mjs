...
function buildContextDigest(removed) {
  ...
  for (const msg of removed) {
    ...
    const firstLine = toContentString(msg.content).split('\n').map(l => l.trim()).find(Boolean) || '';
 ...
 */
export function distillContext(messages, options = {}) {
  if (!messages || messages.length === 0) return [];

  const tailSize = typeof options.tailSize === 'number' ? options.tailSize : 10;
  ...
  for (let i = normalized.length - 1; i >= 0; i--) {
    if (normalized[i].content.includes('"tool": "coder_update_state"')) {
      latestMemoryIdx = i;
      break;
    }
  }
...
export function trimContext(messages, providerId, model) {
  ...
  // ---------------------------------------------------------------------------
  const result = normalizedMessages.map(m => ({ ...m })); // shallow copy each message
