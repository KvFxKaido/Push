/**
 * tui-fuzzy.ts — Fuzzy filtering utilities for the Push TUI.
 ...
 */
export function highlightMatches(text: string, matches: MatchRange[], theme: Theme): string {
...
export function filterSessions(sessions: Session[], query: string): FuzzyFilterResult<Session>[] {
  ...
  const q = query.toLowerCase().trim();
  const results: FuzzyFilterResult<Session>[] = [];
