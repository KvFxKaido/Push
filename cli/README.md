...
| `PUSH_LEAD_RUNTIME` | `kernel` runs interactive turns as a `leadMode` run of the shared coder kernel (`cli/lead-turn.ts` — Agent Runtime Decisions §10 step 2, opt-in). Default: `engine`, the CLI-local loop, until the lane is measured. |
