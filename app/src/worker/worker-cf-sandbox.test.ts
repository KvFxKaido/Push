...
describe('handleCloudflareSandbox happy paths', () => {
  ...
  it('skips over-clone snapshot restore on default-looking branches', async () => {
    // Default branch startup should keep the fresh origin clone fast and boring.
    // A stale or large main snapshot must not make ordinary sandbox creation pay
    // hydrate/guard/re-clone cost before the workspace is usable.
    const sandbox = mockSandbox();
    mockUuid();
    sandbox.exec.mockImplementation(async (command: string) => {
      if (command.includes('merge-base --is-ancestor')) {
        throw new Error('default branch should not run the over-clone guard');
      }
      return { stdout: probeStdout(), stderr: '', exitCode: 0 };
    });
    const r2 = makeR2({
      'cf-snapshots/main-snap': { body: 'QkFTRTY0', customMetadata: { rt: 'rt' } },
    });
    const indexKV = makeSnapshotIndexKV({
      v: 1,
      imageId: 'cf-snapshots/main-snap',
      restoreToken: 'rt',
      repoFullName: 'owner/repo',
      branch: 'main',
      createdAt: 1,
    ...
    const response = await callRoute(
      'create',
      { repo: 'owner/repo', branch: 'main', github_token: 'ghs_token' },
      makeEnv({
        SNAPSHOTS: r2 as unknown as Env['SNAPSHOTS'],
        SNAPSHOT_INDEX: indexKV as unknown as Env['SNAPSHOT_INDEX'],
      }),
