...
import type { ChatMessage, DelegationOutcome, ReasoningBlock, ToolExecutionResult } from '@/types';
...
/**
 * Dispatch the shared tool-budget block path when the grouped turn contains
 * batch overflow or ordering violations. Builds a typed `block` intervention
 * from the shared ledger, then preserves the existing model-facing correction
 * text generated by `handleMultipleMutationsError`.
 */
export function dispatchToolBudgetBlockError(
  detected: DetectedToolCalls,
  round: number,
  accumulated: string,
  thinkingAccumulated: string,
  ...
  const runtimeIntervention = createToolBudgetBlockIntervention(toolLedger, {
    source: 'tool_budget',
