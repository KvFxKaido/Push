...
export async function runRoundLoop(
  ...
  for (let round = 0; ; round++) {
    ...
    if (round === 0) {
      // Hand the bar the thinking-verb pool so it *rotates* (Thinking… /
      // Reasoning…) through pre-response dead air — same presentation as the
      // inline lane (`chat-send-inline.ts`); `phase` is the static event-log
      // fallback.
