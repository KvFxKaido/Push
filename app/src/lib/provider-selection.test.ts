...
const { mockGetModelNameForProvider, mockNormalizeConversationModel } = vi.hoisted(() => ({
...
import { resolveChatProviderSelection, resolveProviderSpecificModel } from './provider-selection';
...
describe('provider selection', () => {
  beforeEach(() => {
    ...
    mockGetModelNameForProvider.mockImplementation(
      (provider: string) => `${provider}-default-model`,
    );
    mockNormalizeConversationModel.mockImplementation(
      (_provider: string, model: string | null | undefined) => {
  ...
  it('ignores explicit model overrides when their provider no longer matches', () => {
    ...
    expect(resolveProviderSpecificModel('openrouter', ' openai/gpt-5 ', undefined)).toBe(
      'openai/gpt-5',
    );
  });
});
