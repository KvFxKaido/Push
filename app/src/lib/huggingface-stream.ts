...
export async function* huggingfaceStream(
  ...
  const apiKey = (getHuggingFaceKey() ?? '').trim();
  ...
  const response = await fetch(PROVIDER_URLS.huggingface.chat, {
    method: 'POST',
    headers,
  ...
  if (!response.ok) {
    ...
    const message = detail.startsWith('Hugging Face ')
      ? detail
  ...
  if (!response.body) {
    throw new Error('Hugging Face response had no body');
  }
