...
// Then map to the wire shape toLLMMessages emits: {role, content, contentBlocks?}
// with the tool sidecars dropped.
...
describe('ollamaStream', () => {
  ...
  it('mixed conversation: plain + paired tool + unpaired result, no contentBlocks leak', async () => {
    ...
    const iter = ollamaStream({ ...baseRequest, messages, tools: [sampleTool] });
    void iter[Symbol.asyncIterator]()
      .next()
      .catch(() => {});
    await new Promise((r) => setTimeout(r, 0));
    ...
    expect(body.messages).toEqual([
      ...
      { role: 'tool', tool_call_id: 'toolu_1', content: 'file body' },
      // Unpaired result degrades to its text form — no half-flatten.
