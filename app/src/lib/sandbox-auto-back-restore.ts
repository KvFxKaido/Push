...
// Both builders interpolate `ref` into a raw shell command. That is safe only
// because `resolveAutoBackRef` rejects anything outside git's `[A-Za-z0-9._/-]`
// ref charset (isInvalidGitRef) before we get here — no shell metacharacters can
// reach this point. Do NOT call these with an unvalidated ref.
...
function applyCommand(ref: string): string {
  return [
    'cd /workspace 2>/dev/null || { echo "ERR"; exit 0; }',
...
export async function detectAutoBackRestore(
  ...
  const first = lines[0] ?? '';
  if (first === 'NONE' || first === 'NOCHANGES' || first === 'ERR') {
    const reason = first.toLowerCase();
    log('info', 'auto_back_restore_none', {
      branch: resolved.branch,
      ref: resolved.ref,
      reason,
...
export async function applyAutoBackRestore(
  ...
  let execError: string | undefined;
  try {
    const result = await execInSandbox(sandboxId, applyCommand(resolved.ref));
    stdout = result.stdout ?? '';
    execError = result.error;
  } catch (err) {
    const reason = err instanceof Error ? err.message : String(err);
