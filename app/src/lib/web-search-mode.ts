 ...
 * (OpenAI, xAI, Sakana Fugu) get OpenAI's `web_search` server tool — all
  ...
  switch (mode) {
    ...
    case 'auto':
      return (
        ...
        // `web_search` built-in tool: direct OpenAI (broad model support),
        // xAI Grok, and Sakana Fugu (every Fugu model ships it). Fireworks is deliberately
