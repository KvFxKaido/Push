...
- sandbox_edit_file(path, edits, expected_version?) — Edit a file using content hashes as line references. edits is an array of HashlineOp: { op: "replace_line" | "insert_after" | "insert_before" | "delete_line", ref: string (7-12 hex char hash prefix), content: string }. Read results show 7-char hashes by default; use those in refs. If an edit fails with an ambiguity error, the error will include longer (12-char) hashes — retry with the longer prefix to disambiguate.
