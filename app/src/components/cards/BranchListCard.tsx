      ...
      <div className={CARD_LIST_CLASS}>
        {data.branches.map((branch) => (
          ...
          >
            ...
            {branch.isDefault && (
              <span className="inline-flex items-center gap-0.5 text-push-xs text-push-link bg-push-link/10 px-1.5 py-0.5 rounded-full shrink-0">
                <Star className="h-2.5 w-2.5" />
                default
              </span>
            )}
            {branch.isProtected && (
              <span className="inline-flex items-center gap-0.5 text-push-xs text-push-status-warning bg-push-status-warning/10 px-1.5 py-0.5 rounded-full shrink-0">
