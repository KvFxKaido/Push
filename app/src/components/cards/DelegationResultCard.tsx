...
export function DelegationResultCard({ data }: { data: DelegationResultCardData }) {
  ...
  return (
    <div className={CARD_SHELL_CLASS}>
      ...
      >
        <div
          className={`flex items-center gap-2.5 border-b border-push-edge px-3.5 py-3 ${status.header}`}
        >
        ...
        <div className="px-3.5 py-3">
          <p className="text-push-base leading-relaxed text-push-fg-secondary">{data.summary}</p>

          <div className="mt-3 flex flex-wrap gap-2">
            {typeof data.fileCount === 'number' && (
              <span
                className={`rounded-full px-2 py-0.5 text-push-2xs font-medium ${CARD_BADGE_INFO}`}
              >
            ...
            {typeof data.checksTotal === 'number' && data.checksTotal > 0 && (
              <span
                className={`rounded-full px-2 py-0.5 text-push-2xs font-medium ${data.checksPassed === data.checksTotal ? CARD_BADGE_SUCCESS : CARD_BADGE_WARNING}`}
              >
            ...
            {auditorVerdict && (
              <span
                className={`rounded-full px-2 py-0.5 text-push-2xs font-medium ${
            ...
            >
              ...
              {data.checkpoints > 0
                ? `, ${data.checkpoints} checkpoint${data.checkpoints === 1 ? '' : 's'}`
                : ''}
            </span>
            <span
              className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-push-2xs font-medium ${CARD_BADGE_INFO}`}
            >
      ...
      <ExpandableCardPanel expanded={expanded && hasDetails}>
        <div className={`px-3.5 py-3 ${CARD_LIST_CLASS}`}>
          {data.verifiedText && (
            <div className="pb-3">
              <div className="mb-1 text-push-xs uppercase tracking-wide text-push-fg-dim">
                Verified
              </div>
              <div
                className={`${CARD_PANEL_SUBTLE_CLASS} px-2.5 py-2 text-push-sm text-push-fg-secondary`}
              >
                {data.verifiedText}
              </div>
            </div>
                ...
                {data.gateVerdicts.map((verdict, index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="mt-0.5 text-push-xs text-push-fg-dim">{verdict.summary}</div>
                    </div>
                  </div>
                ))}
              </div>
