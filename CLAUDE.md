...
Desktop wraps the same PWA via `@capawesome/capacitor-electron` (a Capacitor platform, same wiring model as Android). **Load mode is bundled, unlike Android**: the runtime ignores `server.url` and serves the synced `dist/` copy, so each web deploy needs an `electron:sync`. Why (plus the `CAPACITOR_ELECTRON_DEV_SERVER_URL` dev-mode escape hatch and its relaxed CSP) lives in `app/README.md` "Desktop app" — don't restate it here. Scaffolding path for the "native Windows Electron shell" in [`docs/decisions/Windows Desktop — WSL-Hosted Daemon.md`](docs/decisions/Windows%20Desktop%20—%20WSL-Hosted%20Daemon.md). `app/electron/` is **committed source** mirroring `app/android/` (platform config + `main.ts` + its own `pnpm-lock.yaml`; its `.gitignore` excludes everything regenerated). Don't regenerate via `cap add`; a fresh clone needs only the `--ignore-workspace` install + `electron:sync`.
