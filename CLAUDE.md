...
- **Reviewer** — On-demand advisory diff review in the Workspace Hub. Produces structured findings, line-anchored when possible, and can post them to an open PR as a GitHub review.
...
**Reviewer:** The Reviewer runs on demand from the Workspace Hub `Review` tab. It reviews the current working diff with structured file-level comments, adds line anchors when the model can target specific added lines, and can post those findings back to an open PR as a GitHub PR review (`event: COMMENT`) with inline comments for anchored findings and body bullets for file-level notes.
...
**PR awareness:** Home screen shows open PR count and review-requested indicator. Chat tools include `github_list_prs`, `github_get_pr`, `github_pr_diff`, and `github_list_branches` for reading PR/branch state in any repo. The Workspace Hub `Review` tab can detect an open PR for the active branch and post Reviewer findings back to GitHub.
