...
The TypeScript toolchain is on **TS 7.0 GA** but still split by package. **`cli/` and `mcp/github-server` run TS 7.0 GA** (`typescript@~7.0.2`) — both typecheck *and* emit (`build:cli`, MCP `build`) go through the native `tsc`. (`cli/` has no `package.json`; it compiles with the **root's** `typescript`.) **`app/` stays on TS 7 native-preview typecheck (`tsgo` from `@typescript/native-preview`) + TS 6 (`typescript@^6.0.3`) for ESLint** — it can't fold onto `typescript@7` yet because TS 7.0 GA ships **no** programmatic API (7.1 is expected to introduce a new one) and `typescript-eslint` still hard-caps its `typescript` peer at `<6.1.0` (as of 8.63.0). The app keeps `tsgo` (a *distinct* binary) rather than GA `tsc` on purpose: GA `typescript@7`'s only bin is `tsc`, which would collide in `.bin/tsc` with the app's TS 6 copy. When typescript-eslint supports TS 7, the app drops native-preview and folds onto `typescript@7`; until then the split stands. App emit is `vite build` (esbuild/rollup); `tsgo`/`tsc` only typecheck. `npm run typecheck:all` runs everything (cli/mcp via `tsc`, app via `tsgo`). If `tsgo: not found` (unsupported platform, `--no-optional` install), fall back to `npx tsc -b` for `app/`.
