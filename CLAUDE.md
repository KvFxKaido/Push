...
- **Config-file changes.** Grep new diffs in `wrangler.jsonc`, `*.yml`, `.env*`, anything in `secrets/` for account IDs, slugs, tokens, or hardcoded URLs that should be secrets or dashboard vars. The public repo has bitten this — see [[public-repo-wrangler-vars]] in memory.
...
- **Auth / allowlist seams.** New endpoints or auth gates: trace at least one denied path AND one allowed path end-to-end. Allowlist bypass on PR #508 shipped because only the allowed path was traced. See [[symmetric-security-boundaries-bots-catch]].
