...
**Web search tools:** The Orchestrator can search the web mid-conversation via `web-search-tools.ts`. Three backends: **Tavily** (premium, LLM-optimized results via `VITE_TAVILY_API_KEY`), **Ollama native search** (POST `/api/web_search`), and **DuckDuckGo** (free fallback). In native function-calling mode (Mistral/OpenRouter/Z.AI/Google/Zen), web search is passed via request `tools[]` (`web_search`) rather than the Mistral Agents API path. API keys are configurable at runtime via Settings.
