...
| Item | Status | Scope | Acceptance Criteria |
|---|---|---|---|
| UX: Preserving Context on Branch Creation | done | "New Branch from Here" UI flow plus `sandbox_create_branch` / `sandbox_switch_branch` tools that migrate the active conversation across branch transitions and keep sandbox/Push state in sync. Foreground tools emit `branchSwitch` for UI routing; background coder jobs emit `meta` for observability only. Shipped as slice 2 (#412), slice 2.1 (#413), slice 2.5 (#414), and slice 3 (#415) | A new branch can be created (or switched to) from an active chat without dropping the conversation context or destroying uncommitted work; raw `git checkout` / `git switch` is blocked in `sandbox_exec` and routes through the typed tools |
