...
- **Package manager.** The repo is a **pnpm workspace** (`pnpm-workspace.yaml`): root + `app/` + `mcp/github-server` share **one** `pnpm-lock.yaml`. A single `pnpm install` at the root installs all three — there is no separate `cd app && install` step any more. `package.json` pins `packageManager: pnpm@<version>` (corepack), so use that pnpm and the lockfile won't churn between environments; `git checkout -- pnpm-lock.yaml` is the safe reset. **pnpm stays on 10.x**, not 11 — by choice, not constraint. The old blocker (pnpm 11 needs Node >=22.13; the Cloudflare sandbox base shipped Node 20, and Push's deps get pre-baked there via `Dockerfile.sandbox`) no longer holds: that base image now ships Node v22.23.1, which clears pnpm 11's floor. Bumping is a deliberate change of its own — see `pnpm-workspace.yaml`. Dependency build scripts are blocked by default; the ones that genuinely need a postinstall (esbuild, sharp, onnxruntime-node, protobufjs) are allow-listed in `pnpm-workspace.yaml` under `allowBuilds`.
