...
- **`app/`** stays on **TS 7 native-preview** typecheck (`tsgo` from `@typescript/native-preview`, also platform-specific `optionalDependencies`) plus **TS 6** (`typescript@^6.0.3`) for ESLint (emit is Vite's job — no `tsc` involved). The app can't move to TS 7's `typescript` package yet: TS 7.0 GA ships **no** programmatic API (7.1 is expected to introduce a new one), and `typescript-eslint` still hard-caps its `typescript` peer at `<6.1.0`. It keeps `tsgo` (a distinct binary) rather than GA `tsc` on purpose — GA `typescript@7`'s only bin is `tsc`, which would collide in `.bin/tsc` with the app's TS 6 copy. `cd app && npm run typecheck` covers the two leaf tsconfigs (`tsconfig.app.json`, `tsconfig.node.json`).
