...
Typechecking uses `tsgo` from `@typescript/native-preview` (TypeScript 7.0). The root `npm run typecheck` covers `cli/`, and `cd app && npm run typecheck` covers the app's two leaf tsconfigs (`tsconfig.app.json` and `tsconfig.node.json`); `npm run typecheck:tsgo` at the root runs everything. The native binary ships via platform-specific `optionalDependencies` (linux/darwin/win × x64/arm64). If you install with `--no-optional`, run on an unsupported platform, or otherwise see `tsgo: not found`, fall back to `npx tsc --noEmit` (cli) or `npx tsc -b` (app). Emit (`build:cli`, vite's `build`) still uses `tsc`, so nothing else changes.
