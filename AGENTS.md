...
Backend/model routing is currently split. Settings stores default backend/model picks plus the preferred active backend. `useChat` locks the Orchestrator to a per-chat provider/model on first send, delegated Coder runs inherit that chat-locked provider/model, Reviewer keeps its own sticky provider/model selection, and Auditor follows the same chat-locked provider/model when available, otherwise the active backend/default-provider path.
...
**OpenRouter** provides access to 50+ models through a single API. Push ships with a curated catalog spanning Claude, GPT-4.1/GPT-4o/GPT-5.4, Codex, Cohere Command-R, Gemini, Mistral, MiniMax, Qwen, GLM, DeepSeek, Perplexity Sonar, Arcee Trinity, Mercury, Xiaomi MiMo, Grok, and Kimi.
