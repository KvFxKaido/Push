...
| AI | Multi-backend: built-ins (Ollama, OpenRouter, OpenCode Zen, Blackbox AI, Nvidia NIM, Kilo Code, OpenAdapter) + opt-in private connectors (Azure, Bedrock, Vertex) |
...
Default OpenAdapter model is `deepseek/deepseek-v3`.
...
The Orchestrator can also delegate read-only investigation via `delegate_explorer`. The Explorer uses the same chat-locked provider/model, can inspect GitHub and sandbox read tools plus web search, and returns a concise evidence-based report without editing files or mutating the workspace.
...
### Configuration
Config resolves: CLI flags > env vars > `~/.push/config.json` > defaults. Six providers (Ollama, OpenRouter, OpenCode Zen, Nvidia NIM, Kilo Code, OpenAdapter), all OpenAI-compatible SSE with retry on 429/5xx. All tools are prompt-engineered (JSON blocks in model output, client-side dispatch). CLI web search backend is configurable via `--search-backend`, `PUSH_WEB_SEARCH_BACKEND`, or config (`auto` default: Tavily -> Ollama native -> DuckDuckGo).
...
| `lib/github-tools.ts` | GitHub tool protocol, `delegate_coder`, `fetchProjectInstructions`, branch/merge/PR operations (`executeCreateBranch`, `executeCreatePR`, `executeMergePR`, `executeDeleteBranch`, `executeCheckPRMergeable`, `executeFindExistingPR`, `findOpenPRForBranch`, `fetchGitHubReviewDiff`, `executePostPRReview`) |
